...
class Pa_Validate
{
  ...
  {
    add_action('init', array('pa_validate', 'init'));
    add_action('admin_head', array('pa_validate', 'admin_head'));
    add_action('admin_notices', array('pa_validate', 'notices'));
    add_action('pa_notices', array('pa_validate', 'notices'));
    add_action('wp_ajax_pa_validate', array('pa_validate', 'ajax'));
    add_action('wp_ajax_nopriv_pa_validate', array('pa_validate', 'ajax'));
    add_action('wp_loaded', array('pa_validate', 'set_data'), 99999);

    add_filter('pa_assets_localize', array('pa_validate', 'assets_localize'));
    add_filter('wp_redirect', array('pa_validate', 'wp_redirect'), 10, 2);
    add_filter('pa_validation_rules', array('pa_validate', 'validation_rules'));
    add_filter('pa_sanitization_rules', array('pa_validate', 'sanitization_rules'));
  }
  ...
  public static function init()
  {
     ...
     * @since 1.0
     */
    self::$validation_rules = apply_filters('pa_validation_rules', self::$validation_rules);

    /**
     * pa_sanitization_rules
     * Add your own sanitization rules.
     *
     * @since 1.0
     */
    self::$sanitization_rules = apply_filters('pa_sanitization_rules', self::$sanitization_rules);
  ...
  public static function wp_redirect($location, $status)
  {
    ...
    $parameter = pa::$prefix . '[' . self::$parameter . ']';
  ...
  public static function admin_head()
  {
    ...
    {
      pa::render('shared/notice-updated-hide');
    }
  }
    ...
    switch ($method)
    {
      case 'check':
        ...
        if (isset($_REQUEST['data']))
        {
          ...
          array_walk_recursive($data, array('pa', 'array_values_strip_all_tags'));

          extract($data[pa::$prefix]);

          if ($nonce && $fields && wp_verify_nonce($nonce, 'pa-' . $fields))
          {
  ...
  public static function notices($form_id = null, $fetch = false)
  {
    ...
    $submitted_form_id = pa_form::get('form_id');
    ...
    if (((($submitted_form_id && $form_id == $submitted_form_id) || !$submitted_form_id) && !empty($errors)) || $fetch)
    {
      ...
      if ($fetch)
      {
        return pa::render('shared/notice', $arguments, true);
      }
      else
      {
        pa::render('shared/notice', $arguments);
      }
    }
  }
  ...
  public static function check($stored_data = null, $fields_id = null)
  {
    ...
    if (!$fields_id)
    {
      $fields_id = isset($_REQUEST[pa::$prefix]['fields']) ? esc_attr($_REQUEST[pa::$prefix]['fields']) : null;
    }

    if (!$fields_id || !$fields_data = get_transient(pa::$prefix . $fields_id))
    {
      return false;
    }
    ...
    foreach ($fields_data as $type => &$fields)
    {
      foreach ($fields as &$field)
      {
        ...
        {
          if ($field['prefix'] && isset($stored_data[pa::$prefix . $field['scope']]))
          {
            $request_data = $stored_data[pa::$prefix . $field['scope']];
          }
        ...
        {
          if (($scope_prefix = pa_form::is_related_field($field)) != false && isset($_REQUEST[pa::$prefix . $scope_prefix]))
          {
            $request_data = $_REQUEST[pa::$prefix . $scope_prefix];
          }
          elseif ($field['scope'] && isset($_REQUEST[pa::$prefix . $field['scope']]))
          {
            $request_data = $_REQUEST[pa::$prefix . $field['scope']];
          }
          elseif ($field['scope'] && $field['scope'] == pa::$prefix && isset($_REQUEST[pa::$prefix]))
          {
            $request_data = $_REQUEST[pa::$prefix];
          }
          else
          {
        ...
        if ($request_data && $field['field'] && $field['type'] != 'html')
        {
          if (!in_array($field['field'], $clones))
          {
            ...
            elseif (strstr($field['field'], ':'))
            {
              ...
              {
                $request_data = pa::array_path_get($request_data, explode(':', $pluck));
                ...
                {
                  $field['request_value'] = $request_data ? pa::pluck($request_data, $pluck_field) : null;
                }
              }
            }
          }
          ...
          {
            $paths = pa::array_paths($field['request_value']);
            ...
            foreach ($paths as $path)
            {
              ...
              if (!isset($fields[$path]) && !in_array($field_name, $clones))
              {
                ...
                $clone = pa_form::get_fields_data($fields_data, $field['scope'], $original);
                ...
                if ($clone)
                {
                  ...
                  $exists = pa_form::get_fields_data($fields_data, $field['scope'], $field_name);

                  if ($exists)
                  {
                    $fields_data = pa_form::update_fields_data($fields_data, $exists, null, 'request_value', pa::array_path_get($field['request_value'], $path));
                  }
                  else
                  {
                    $clone['field'] = $field_name;
                    $clone['id'] = pa_form::get_field_id($clone);
                    $clone['name'] = pa_form::get_field_name($clone);
                    $clone['request_value'] = pa::array_path_get($field['request_value'], $path);
            ...
            if (isset(self::$sanitization_rules[$sanitize['type']]))
            {
              ...
              if (isset($sanitization['callback']))
              {
                ...
                if (strstr($field['field'], ':'))
                {
                  ...
                  $group_field = pa_form::get_fields_data($fields_data, $field['scope'], $group);

                  if (pa::array_path_get($group_field['request_value'], $path))
                  {
                    pa::array_path_set($group_field['request_value'], $path, $field['request_value']);
                  }

                  pa_form::update_fields_data($fields_data, $group_field, null, 'request_value', $group_field['request_value']);
                }
              }
            }
            else
            {
              $trigger_error = sprintf(__('Sanitization type "%s" is not valid.', 'pa'), $sanitize['type']);
          ...
          foreach ($field['validate'] as $validate)
          {
            ...
            {
              $trigger_error = sprintf(__('Validation type "%s" is not valid.', 'pa'), $validate['type']);
    ...
    if (pa_admin::is_widget() || pa_admin::is_setting())
    {
      self::set_data();
    }
  ...
  /**
   * assets_localize
   * Add data to the local pa variable
   *
   * @param array $localize The data being passed to the pa javascript object.
   *
  ...
  public static function assets_localize($localize)
  {
    ...
    $localize['validate'] = pa::get_settings('pa_core', 'form_validate_js') ? true : false;
  ...
  {
    $message = is_string($field['required']) ? $field['required'] : __('is a required field.', 'pa');

    if ($field['type'] == 'file' && isset($_FILES[pa::$prefix . $field['scope']]['name'][$field['field']]))
    {
      $index = 0;

      foreach ($_FILES[pa::$prefix . $field['scope']]['error'][$field['field']] as $error)
      {
        if ($error != 0)
        {
    ...
    {
      if($field["type"] == "number" && $field['request_value'] == 0){
      }else{
  ...
  private static function validate_value_callback($field, $validation, $fields_data)
  {
    ...
    if (is_array($field['request_value']) && ($field['add_more'] || !pa::is_flat($field['request_value'])) && $field['type'] != 'group')
    {
      $index = 0;
      ...
      foreach ($field['request_value'] as $request_value)
      {
        if ($field['request_value'][$index]) {
          ...
          {
            $field = self::add_error($field, $index, !empty($validation['message']) ? $validation['message'] : (is_string($validation_result) ? $validation_result : __('is not valid input', 'pa')));
          }
        }
    ...
    elseif ($field['request_value'])
    {
      ...
      if ($validation_result !== true)
      {
        $field = self::add_error($field, 0, !empty($validation['message']) ? $validation['message'] : (is_string($validation_result) ? $validation_result : __('is not valid input', 'pa')));
      }
    }
  ...
  public static function set_data()
  {
    if (!self::$valid)
    {
      ...
      $set = set_transient(pa::$prefix . 'validation_' . self::$id, self::$processed['fields_data']);
  ...
  {
    if (isset($_REQUEST[pa::$prefix]) && isset($_REQUEST[pa::$prefix][self::$parameter]))
    {
      self::$id = esc_attr($_REQUEST[pa::$prefix][self::$parameter]);

      self::$form_submission = get_transient(pa::$prefix . 'validation_' . self::$id);

      delete_transient(pa::$prefix . 'validation_' . self::$id);
    }
  }
  ...
  {
    if (null !== ($submission = pa_form::get_fields_data(self::$form_submission, $field['scope'], $field['field'], pa_form::is_related_field($field))))
    {
  ...
  public static function validation_rules($validation_rules)
  {
    $validation_rules = array_merge($validation_rules, array(
      'email' => array(
        'name' => __('Email Address', 'pa')
        ,'description' => __('Verifies that the input is in the proper format for an email address.', 'pa')
        ,'callback' => array('pa_validate', 'validate_email')
      )
      ,'email_domain' => array(
        'name' => __('Email Domain', 'pa')
        ,'description' => __('Verifies that the email domain entered is a valid domain.', 'pa')
        ,'callback' => array('pa_validate', 'validate_email_domain')
      )
      ,'email_exists' => array(
        'name' => __('Email exists?', 'pa')
        ,'description' => __('Checks that the entered email is not already registered to another user.', 'pa')
        ,'callback' => array('pa_validate', 'validate_email_exists')
      )
      ,'file_exists' => array(
        'name' => __('File Exists?', 'pa')
        ,'description' => __('Verifies that the file path entered leads to an actual file.', 'pa')
        ,'callback' => array('pa_validate', 'validate_file_exists')
      )
      ,'hex_color' => array(
        'name' => __('Hex Color', 'pa')
        ,'description' => __('Verifies that the data entered is a valid hex color.', 'pa')
        ,'callback' => array('pa_validate', 'validate_hex_color')
      )
      ,'image' => array(
        'name' => __('Is Image?', 'pa')
        ,'description' => __('Verifies that the file path entered leads to an image file.', 'pa')
        ,'callback' => array('pa_validate', 'validate_image')
      )
      ,'ip_address' => array(
        'name' => __('IP Address', 'pa')
        ,'description' => __('Verifies that the data entered is a valid IP Address.', 'pa')
        ,'rule' => "/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/"
        ,'message' => __('is not a valid ip address.', 'pa')
      )
      ,'limit' => array(
        'name' => __('Entry Limit', 'pa')
        ,'description' => __('Verifies that the number of items are within the defined limit.', 'pa')
        ,'callback' => array('pa_validate', 'validate_limit')
      )
      ,'range' => array(
        'name' => __('Range', 'pa')
        ,'description' => __('Verifies that the data entered is within the defined range.', 'pa')
        ,'callback' => array('pa_validate', 'validate_range')
      )
      ,'safe_text' => array(
        'name' => __('Alphanumeric', 'pa')
        ,'description' => __('Verifies that the data entered is alphanumeric.', 'pa')
        ,'rule' => "/^[a-zA-Z0-9 .-]+$/"
        ,'message' => __('contains invalid characters. Must contain only letters and numbers.', 'pa')
      )
      ,'url' => array(
        'name' => __('URL', 'pa')
        ,'description' => __('Verifies that the data entered is a valid URL.', 'pa')
        ,'rule' => "/^(https?:\/\/)?([\da-z\.\-]+)\.([a-z\.]{2,6})([\/\w \.\-@!+]*)*\/?$/"
        ,'message' => __('is not a valid url.', 'pa')
      )
      ,'username_exists' => array(
        'name' => __('Username exists?', 'pa')
        ,'description' => __('Checks that the entered username does not already exist.', 'pa')
        ,'callback' => array('pa_validate', 'validate_username_exists')
      )
      ,'match' => array(
        'name' => __('Match Fields', 'pa')
        ,'description' => __('Checks to see if two fields match.', 'pa')
        ,'callback' => array('pa_validate', 'validate_match')
      )
    ));
  ...
  {
    return is_email($value) ? true : __('does not contain a valid Email Address.', 'pa');
  }
  ...
  {
    return (bool) @checkdnsrr(preg_replace('/^[^@]++@/', '', $value), 'MX') ? true : __('does not contain a valid Email Domain.', 'pa');
  }
  ...
  public static function validate_email_exists($index, $value, $options, $field, $fields)
  {
    ...
    return (email_exists($value) && !is_user_logged_in()) || (email_exists($value) && is_user_logged_in() && $value != $current_user->user_email) ? sprintf(__('cannot be "%s". This email is registered to another user.', 'pa'), $value) : true;
  }
  ...
  public static function validate_file_exists($index, $value, $options, $field, $fields)
  {
    ...
    foreach ($field_value as $value)
    {
      ...
      {
        return __('contains a file that does not exist.', 'pa');
      }
    }
  ...
  public static function validate_image($index, $value, $options, $field, $fields)
  {
    ...
    foreach ($field_value as $value)
    {
      if ($value)
      {
        ...
        {
          return __('contains a file that is not an image.', 'pa');
        }
      }
    }
   ...
   *
   * Request value can be any Pa field.
   *
   * @param int $index The field index being checked.
   * @param mixed $value The value of the field.
   * @param array $options The options.
  ...
  public static function validate_limit($index, $value, $options, $field, $fields)
  {
    ...
    switch ($count)
    {
      ...
      default:
        ...
        $grammar = $field['type'] == 'file' || $field['add_more'] ? __('items added', 'pa') : __('items selected', 'pa');
    ...
    if ($total < $min || $total > $max)
    {
      if ($min == $max)
      {
        return sprintf(__('must have exactly %1$s %2$s.', 'pa'), $min, $grammar);
      }
      else
      {
        return sprintf(__('must have between %1$s and %2$s %3$s.', 'pa'), $min, $max, $grammar);
      }
    }
  ...
  public static function validate_range($index, $value, $options, $field, $fields)
  {
    ...
    {
      return sprintf(__('contains a value that is not between %s and %s', 'pa'), $min, $max);
    }
  }
  ...
  public static function validate_username_exists($index, $value, $options, $field, $fields)
  {
    ...
    {
      return sprintf(__('cannot be "%s". This username already exists.', 'pa'), $value);
    }
    else
    {
      return true;
  ...
  public static function validate_match($index, $value, $options, $field, $fields)
  {
    if (isset($options['field']))
    {
      ...
      {
        $match_field = pa_form::get_fields_data($fields, $scope, $group);
        ...
        {
          return sprintf(__('must match <strong>%s</strong>', 'pa'), isset($match_field['label']) ? $match_field['label'] : $match_field['field'], $value);
        }
      }
    }
  ...
  public static function sanitization_rules($sanitization_rules)
  {
    $sanitization_rules = array_merge($sanitization_rules, array(
      'email' => array(
        'name' => __('Email address', 'pa')
        ,'description' => __('Strips out all characters that are not allowable in an email address.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_email')
      )
      ,'file_name' => array(
        'name' => __('File name', 'pa')
        ,'description' => __('Removes or replaces special characters that are illegal in filenames.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_file_name')
      )
      ,'html_class' => array(
        'name' => __('HTML class', 'pa')
        ,'description' => __('Removes all characters that are not allowable in an HTML classname.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_html_class')
      )
      ,'text_field' => array(
        'name' => __('Text field', 'pa')
        ,'description' => __('Removes all HTML markup, as well as extra whitespace, leaving only plain text.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_text_field')
      )
      ,'title' => array(
        'name' => __('Post title', 'pa')
        ,'description' => __('Removes all HTML and PHP tags, returning a title that is suitable for a url', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_title')
      )
      ,'user' => array(
        'name' => __('Username', 'pa')
        ,'description' => __('Removes all unsafe characters for a username.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_user')
      )
      ,'wp_kses' => array(
        'name' => __('wp_kses', 'pa')
        ,'description' => __('Makes sure that only the allowed HTML element names, attribute names and attribute values plus only sane HTML entities are accepted.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_wp_kses')
      )
      ,'wp_filter_kses' => array(
        'name' => __('wp_filter_kses', 'pa')
        ,'description' => __('Makes sure only default HTML elements are accepted.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_wp_filter_kses')
      )
      ,'wp_kses_post' => array(
        'name' => __('wp_kses_post', 'pa')
        ,'description' => __('Makes sure only appropriate HTML elements for post content are accepted.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_wp_kses_post')
      )
      ,'wp_strip_all_tags' => array(
        'name' => __('wp_strip_all_tags', 'pa')
        ,'description' => __('Properly strip all HTML tags including script and style.', 'pa')
        ,'callback' => array('pa_validate', 'sanitize_wp_strip_all_tags')
      )
    ));
