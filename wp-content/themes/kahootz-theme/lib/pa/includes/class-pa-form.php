...
/**
 * Pa_Form
 * Manages fields and forms for Pa.
 *
 * @package     Pa
 * @subpackage  Form
...
class Pa_Form
{
  ...
  private static $field_assets = array(
    'colorpicker' => array(
      'callback' => array('pa_form', 'render_field_custom_assets')
    )
    ,'datepicker' => array(
      'scripts' => array(
  ...
  {
    add_action('wp_loaded', array('pa_form', 'wp_loaded'), 100);

    add_action('post_edit_form_tag', array('pa_form', 'add_enctype'));
    add_action('user_edit_form_tag', array('pa_form', 'add_enctype'));
    add_action('edit_category_form_pre', array('pa_form', 'setup_add_enctype'));
    add_action('edit_link_category_form_pre', array('pa_form', 'setup_add_enctype'));
    add_action('edit_tag_form_pre', array('pa_form', 'setup_add_enctype'));
    add_action('created_term', array('pa_form', 'created_term'), 10000, 3);

    add_action('init', array('pa_form', 'save_fields_actions'), 100);
    add_action('init', array('pa_form', 'register_forms'));
    add_action('wp_ajax_pa_form', array('pa_form', 'ajax'));
    add_action('wp_ajax_nopriv_pa_form', array('pa_form', 'ajax'));
    add_action('admin_enqueue_scripts', array('pa_form', 'wp_enqueue_media'));
    add_action('admin_footer', array('pa_form', 'render_field_assets'));
    add_action('wp_footer', array('pa_form', 'render_field_assets'));
    add_action('customize_controls_print_footer_scripts', array('pa_form', 'render_field_assets'));

    add_action('pa_notices', array('pa_form', 'notices'));
    ...
    add_filter('teeny_mce_before_init', array('pa_form', 'tiny_mce_settings'), 100, 2);
    add_filter('tiny_mce_before_init', array('pa_form', 'tiny_mce_settings'), 100, 2);
    add_filter('quicktags_settings', array('pa_form', 'quicktags_settings'), 100, 2);
    add_filter('pa_field_templates', array('pa_form', 'field_templates'), 0);
    add_filter('the_editor', array('pa_form', 'the_editor'));
    add_filter('tiny_mce_before_init', array('pa_form', 'remove_theme_css'), 10, 2);
    add_filter('teeny_mce_before_init', array('pa_form', 'remove_theme_css'), 10, 2);
  }
  ...
  public static function wp_loaded()
  {
     ...
     * @since 1.0
     */
    self::$templates = apply_filters('pa_field_templates', self::$templates);
     ...
     * @since 1.0
     */
    $pa_field_list_types = apply_filters('pa_field_list_types', array());

    // Do not allow filter to overwrite default $field_list_types
    self::$field_list_types = array_merge_recursive($pa_field_list_types, self::$field_list_types);
     ...
     * @since 1.0
     */
    $pa_field_alias = apply_filters('pa_field_alias', array());

    // Do not allow filter to overwrite default $field_alias
    self::$field_alias = array_merge($pa_field_alias, self::$field_alias);

    foreach (self::$template_shortcodes as $template_shortcode)
    {
      add_shortcode($template_shortcode, array('pa_form', 'template_shortcode'));
    }
    ...
    if (in_array($pagenow, array('widgets.php', 'customize.php')))
    {
      ...
      if ($pagenow == 'widgets.php')
      {
        ...
        add_action('admin_footer', array('pa_form', 'editor_proxy'));
      }
      else
      {
        add_action('customize_controls_print_footer_scripts', array('_WP_Editors', 'editor_js'), 50);
        add_action('customize_controls_print_footer_scripts', array('pa_form', 'editor_proxy'));
      }
    }
  ...
  {
    if (isset($_REQUEST[pa::$prefix]['nonce']) && isset($_REQUEST[pa::$prefix]['fields']))
    {
      self::$nonce = wp_verify_nonce($_REQUEST[pa::$prefix]['nonce'], 'pa-' . $_REQUEST[pa::$prefix]['fields']);
    }
  }
  ...
  public static function field_templates($templates)
  {
    ...
    $form_table = '<table class="form-table pa-form-table">
                     [field_wrapper]
                       <tr class="%1$s">
                         <th scope="row">
                           [field_label]
    ...
    $responsive = '[field_wrapper]
                     <div class="%1$s pa-field-container">
                       <div class="pa-field-container-row">
                         <div class="pa-label-container">
                           [field_label]
                           [field_description_wrapper]
                             <p class="pa-field-description description">[field_description]</p>
                           [/field_description_wrapper]
                         </div>
                         <div class="pa-field">
                           [field]
    ...
    $theme = '[field_wrapper]
                <div class="%1$s pa-theme-field-container">
                  <div class="pa-theme-label">
                    [field_label]
                  </div>
                  <div class="pa-theme-field">
                    [field]
                    [field_description_wrapper]
                      <p class="pa-theme-field-description">[field_description]</p>
                    [/field_description_wrapper]
                  </div>
                </div>
              [/field_wrapper]';
    ...
    $templates['field'] = array(
      'name' => __('Field', 'pa')
      ,'description' => __('Displays a field.', 'pa')
      ,'template' => '[field]'
    );

    $templates['field_label'] = array(
      'name' => __('Field and Label', 'pa')
      ,'description' => __('Displays a field then the label.', 'pa')
      ,'template' => '[field][field_label]'
    );

    $templates['label_field'] = array(
      'name' => __('Label and Field', 'pa')
      ,'description' => __('Displays a label then the field.', 'pa')
      ,'template' => '[field_label][field]'
    );

    $templates['field_description'] = array(
      'name' => __('Field and Description', 'pa')
      ,'description' => __('Displays a field then the description.', 'pa')
      ,'template' => '[field]
                      [field_description_wrapper]
                        <p class="pa-theme-field-description">[field_description]</p>
                      [/field_description_wrapper]'
    ...
    $templates['default'] = array(
      'name' => __('Default', 'pa')
      ,'description' => __('General responsive layout with a fixed label column.', 'pa')
      ,'template' => $responsive
    );

    $templates['submit'] = array(
      'name' => __('Submit Button', 'pa')
      ,'description' => __('General layout for submit button in WordPress.', 'pa')
      ,'template' => $submit
    );

    $templates['form_table'] = array(
      'name' => __('Form Table', 'pa')
      ,'description' => __('Default layout for the WordPress form table.', 'pa')
      ,'template' => $form_table
    );

    $templates['form_table_field'] = array(
      'name' => __('Form Table - Field', 'pa')
      ,'description' => __('The field component of the WordPress form table layout.', 'pa')
      ,'template' => $form_table_field
    );

    $templates['form_table_inline_description'] = array(
      'name' => __('Form Table - Inline Description', 'pa')
      ,'description' => __('The description is just added as appended text.', 'pa')
    ...
    $templates['widget'] = array(
      'name' => __('Widget', 'pa')
      ,'description' => __('Default layout for Widget fields.', 'pa')
      ,'template' => $responsive
    );

    $templates['post_meta'] = array(
      'name' => __('Post', 'pa')
      ,'description' => __('Default layout for Post fields.', 'pa')
      ,'template' => $responsive
    );

    $templates['term_meta'] = array(
      'name' => __('Terms', 'pa')
      ,'description' => __('Default layout for Term fields.', 'pa')
      ,'template' => $form_table
    );

    $templates['term_meta_new'] = array(
      'name' => __('New Terms', 'pa')
      ,'description' => __('Default layout for New Term fields.', 'pa')
      ,'template' => $term_meta_new
    );

    $templates['user_meta'] = array(
      'name' => __('User', 'pa')
      ,'description' => __('Default layout for User fields.', 'pa')
      ,'template' => $form_table
    );

    $templates['option'] = array(
      'name' => __('Option', 'pa')
      ,'description' => __('Default layout for Option fields.', 'pa')
      ,'template' => $responsive
    );

    $templates['shortcode'] = array(
      'name' => __('Shortcode', 'pa')
      ,'description' => __('Default layout for Shortcode fields.', 'pa')
      ,'template' => $responsive
    );

    $templates['media_meta'] = array(
      'name' => __('Media', 'pa')
      ,'description' => __('Default layout for Media fields.', 'pa')
      ,'template' => $responsive
    );

    $templates['theme'] = array(
      'name' => __('Theme', 'pa')
      ,'description' => __('Default layout for frontend fields.', 'pa')
      ,'template' => $theme
    );
  ...
  public static function get_field_id($field)
  {
    ...
    $field['prefix'] = $field['scope'] && $field['prefix'] ? pa::$prefix : null;
    $context = self::get_field_context($field, true);

    if (pa_admin::is_widget() && (!$field['scope'] || ($field['scope'] && ($field['scope'] != pa::$prefix && $field['field'] != 'fields'))))
    {
      $id = pa_widget::widget()->get_field_id(str_replace(':', '_', $field['field']))
    ...
    else
    {
      ...
      if (!is_numeric($field['index']) && strstr($field['field'], ':'))
      {
        ...
        $parts = array_filter($parts, array('pa', 'is_not_numeric'));

        $field_name = implode(':', $parts);
      }

      $id = $field['prefix']
            . ($field['scope'] && $field['scope'] != pa::$prefix ? $context . '_' : null)
  ...
  public static function get_field_name($field)
  {
    ...
    $prefix = !in_array($field['scope'], array(pa::$prefix, false)) && $field['prefix'] ? pa::$prefix : null;
    $context = self::get_field_context($field, true);
    ...
    $use_object = (($multiple && (count($field_choices) > 1 || !$field_choices)) || $ungrouped_add_more || $add_more) && $field['scope'] != pa::$prefix;

    if (pa_admin::is_widget() && (!$field['scope'] || ($field['scope'] && ($field['scope'] != pa::$prefix && $field['field'] != 'fields'))))
    {
      $name = pa_widget::widget()->get_field_name(str_replace(':', '][', $field['field']))
    ...
    {
      $name = $prefix
              . ($field['scope'] ? $context . (pa_admin::is_media() && isset($GLOBALS['pa_attachment']) ? '_' . $GLOBALS['pa_attachment']->ID : '') : null)
  ...
  public static function get_field_object_id($field)
  {
    ...
    switch ($field['scope'])
    {
      ...
      case 'taxonomy':
        ...
        {
          if (isset($_REQUEST[pa::$prefix . 'user']['ID']) && self::valid())
          {
            $id = (int) $_REQUEST[pa::$prefix . 'user']['ID'][0];
          }
        ...
        {
          if (isset($GLOBALS['pa_attachment']))
          {
            $id = $GLOBALS['pa_attachment']->ID;
          }
          else
          {
            if (isset($_REQUEST[pa::$prefix . 'post']['ID']))
            {
              $id = (int) $_REQUEST[pa::$prefix . 'post']['ID'];
            }
      ...
      case 'user_meta':
        ...
        if (isset($_REQUEST[pa::$prefix . 'user']['ID']))
        {
          $id = (int) $_REQUEST[pa::$prefix . 'user']['ID'];
        }
        elseif ($pagenow == 'user-edit.php')
        {
          $id = $user_id;
  ...
  public static function get_field_value($scope, $field, $type, $id = false, $unique = false)
  {
    ...
    if (pa_admin::is_widget())
    {
      return isset(pa_widget::widget()->instance[$field['field']]) ? maybe_unserialize(pa_widget::widget()->instance[$field['field']]) : $field['value'];
    }

    $key = $field['field'] ? $field['field'] : null;
    $prefix = !in_array($scope, array(pa::$prefix, false)) ? pa::$prefix : null;
      ...
      switch ($type)
      {
        ...
        case 'option':
          ...
          if ($type != $scope)
          {
            ...
            if (stristr($key, ':'))
            {
              $value = pa::array_path_get($options, explode(':', $key));
            }
            else
            {
        ...
        case 'taxonomy':
          ...
          /**
           * pa_taxonomy_value_key
           *
           * @since 1.0
           */
          $terms = pa(wp_get_object_terms($id, $key), apply_filters('pa_taxonomy_value_key', 'term_id', $key));
           ...
           * @since 1.0
           */
          $terms = apply_filters('pa_taxonomy_value', $terms, $id, $key, $field);
    ...
    elseif (!$id && $key)
    {
      if (isset($_REQUEST[pa::$prefix . $key]))
      {
        $request_value = $_REQUEST[pa::$prefix . $key];
      }
      elseif (isset($_REQUEST[pa::$prefix . $scope][$key]))
      {
        $request_value = $_REQUEST[pa::$prefix . $scope][$key];
      }
  ...
  public static function get_field_template($scope = null)
  {
    ...
    else
    {
      ...
      elseif (pa_admin::is_media())
      {
        $wrapper = 'media_meta';
      }
      elseif (pa_admin::is_widget())
      {
        $wrapper = 'widget';
      }
      elseif ($type = pa_admin::is_term())
      {
        $wrapper = 'term_meta' . ($type === 'new' ? '_new' : '');
      }
      elseif (pa_admin::is_user())
      {
        $wrapper = 'user_meta';
      }
      elseif (pa_admin::is_widget())
  ...
  public static function setup_add_enctype()
  {
    ...
    add_action($taxnow . '_term_new_form_tag', array('pa_form', 'add_enctype'));
    add_action($taxnow . '_term_edit_form_tag', array('pa_form', 'add_enctype'));
  }
    ...
    switch ($method)
    {
      case 'field':
        ...
        if (isset($_REQUEST['field']) && is_array($_REQUEST['field']))
        {
          ...
          array_walk_recursive($field, array('pa', 'array_values_strip_all_tags'));
          array_walk_recursive($field, array('pa', 'array_values_cast'));
          ...
          if ($widget)
          {
            ...
            $wp_widget_factory->widgets[pa::slug($widget)]->setup(pa::slug($widget));
          }
  ...
  public static function render_field($field, $return = false)
  {
     ...
     * @since 1.0
     */
    $field = apply_filters('pa_request_field', $field);

    /**
     * pa_request_field_$scope_$field
     ...
     * @since 1.0
     */
    $field = apply_filters('pa_request_field_' . $field['scope'] . '_' . $field['field'], $field);
    ...
    {
      $field['relate']['field'] = pa::unique_id();
      $field['relate_to'] = self::get_field_object_id($field);

      if ($field['type'] != 'group')
      {
        $field['field'] = '_' . pa::$prefix . 'relate_' . $field['relate']['scope'];
      }
    }
    ...
    {
      $field = pa_relate::relate_field($field);
    }
    ...
    if ($field['type'] == 'hidden' && !array_key_exists('data-pa-field-addmore-clear', $field['attributes']))
    {
      $field['attributes']['data-pa-field-addmore-clear'] = false;
    }

    // Add default classes
    array_push($field['attributes']['class'], self::get_field_id(array_merge($field, array('index' => false))));
    array_push($field['attributes']['class'], 'pa-field-type-' . $field['type']);
    array_push($field['attributes']['class'], 'pa-field-element');

    // Set Columns
    if (is_numeric($field['columns']) && !$field['child_field'])
    {
      array_push($field['attributes']['wrapper_class'], 'pa-field-type-group pa-field-column-' . $field['columns']);
    }

    if (is_numeric($field['columns']))
    {
      $field['attributes']['data-pa-field-columns'] = $field['columns'];
    }

    if (isset($field['attributes']['columns']) && is_numeric($field['attributes']['columns']))
    {
      array_push($field['attributes']['class'], 'pa-field-column-' . $field['attributes']['columns']);
    ...
    if (!empty($field['on_post_status']))
    {
      ...
      if ($object)
      {
        $status_list = isset($object['post_type']) ? pa_cpt::get_post_statuses($object['post_type']) : array();
    ...
    // Get errors
    if (pa_validate::errors() && !$field['group_field'])
    {
      $field['errors'] = pa_validate::get_errors($field);
    }

    // Highlight errors if needed.
    if ($field['errors'])
    {
      array_push($field['attributes']['class'], 'pa-error');
    }
    ...
    if (!$field['group_field']
        ...
        && $field['scope'] != pa::$prefix
        && !$field['new'])
    {
      if (pa_validate::errors())
      {
        $field['value'] = pa_validate::get_request_value($field);
      }
      elseif (!pa_validate::errors() && (!$field['relate'] || ($field['relate'] && substr($field['field'], 0, strlen('_' . pa::$prefix . 'relate_')) != '_' . pa::$prefix . 'relate_')))
      {
    ...
    {
      array_push($field['attributes']['class'], 'pa-field-element-condition');
      ...
      foreach ($field['conditions'] as &$condition)
      {
        if (is_array($condition))
        {
          ...
          if (isset($condition['field']))
          {
            ...
            if (!in_array('pa-field-condition', $field['attributes']['class']))
            {
              if (!in_array('pa-field-condition', $field['attributes']['wrapper_class']))
              {
                array_push($field['attributes']['wrapper_class'], 'pa-field-condition');
              }

              if (!in_array('pa-field-condition-' . $condition['type'], $field['attributes']['wrapper_class']))
              {
                array_push($field['attributes']['wrapper_class'], 'pa-field-condition-' . $condition['type']);
              }
            }
          }
        }
    ...
    {
      $field['attributes']['data-pa-field-addmore'] = $field['add_more'];
      $field['attributes']['data-pa-field-sortable'] = $field['sortable'];
      $field['attributes']['data-pa-field-addmore-actions'] = $field['add_more'];
      $field['attributes']['data-pa-field-addmore-single'] = $field['add_more'];
    }
    ...
    if ($field['type'] == 'editor')
    {
      ...
      {
        if (substr($key, 0, strlen('data-pa-field-')) == 'data-pa-field-')
        {
     ...
     * @since 1.0
     */
    $field = apply_filters('pa_pre_render_field', $field);

    /**
     * pa_pre_render_field_$scope_$field
     ...
     * @since 1.0
     */
    $field = apply_filters('pa_pre_render_field_' . $field['scope'] . '_' . $field['field'], $field);
    ...
    $field_name = $field['field'] ? $field['field'] : pa::unique_id();
     ...
     * @since 1.0
     */
    $rendered_field = apply_filters('pa_post_render_field', $rendered_field, $field);

    /**
     * pa_post_render_field_$scope_$field
     ...
     * @since 1.0
     */
    $rendered_field = apply_filters('pa_post_render_field_' . $field['scope'] . '_' . $field['field'], $rendered_field, $field);
  ...
  public static function validate_field_parameter($parameter, $value)
  {
    ...
    switch ($parameter)
    {
      case 'capability':
        ...
        return empty($value) || pa_user::current_user_can($value);

      break;

      case 'role':

        return empty($value) || pa_user::current_user_role($value);
      ...
      default:
         ...
         * @since 1.0
         */
        return apply_filters('pa_validate_field_parameter', true, $parameter, $value);
  ...
  public static function save_fields($object = null)
  {
    if (!empty(self::$fields_rendered))
    {
      ...
      do_action('pa_save_fields', self::$fields_rendered);

      // Generate a unique id for this field configuration
      $fields_id = md5(serialize(self::$fields_rendered));

      // Save configuration for later use
      if (false === get_transient(pa::$prefix . $fields_id))
      {
        if (false === set_transient(pa::$prefix . $fields_id, self::$fields_rendered, 60 * 60 * 24) && wp_using_ext_object_cache())
        {
          wp_using_ext_object_cache(false);

          set_transient(pa::$prefix . $fields_id, self::$fields_rendered, 60 * 60 * 24);
      ...
      pa('field', array(
        ...
        ,'scope' => pa::$prefix
        ,'field' => 'nonce'
        ,'value' => wp_create_nonce('pa-' . $fields_id)
      ));
      ...
      pa('field', array(
        ...
        ,'value' => $fields_id
        ,'attributes' => array(
          'data-pa-fields' => json_encode(self::$fields_rendered, JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_QUOT)
        )
      ));
  ...
  public static function save_fields_actions()
  {
    ...
    {
      add_action($action, array('pa_form', 'save_fields'), 101);
    }

    $taxonomies = get_taxonomies('', 'names');
    foreach ($taxonomies as $taxonomy)
    {
      add_action($taxonomy . '_add_form', array('pa_form', 'save_fields'), 101);
      add_action($taxonomy . '_edit_form', array('pa_form', 'save_fields'), 101);
    }
  }
  ...
  public static function render_field_assets()
  {
     ...
     * @since 1.0
     */
    $field_assets = apply_filters('pa_field_assets', self::$field_assets);

    $field_types_rendered = pa_admin::is_widget() ? array_keys($field_assets) : self::$field_types_rendered;

    if (!empty($field_types_rendered))
    {
      $jquery_ui_core = $wp_scripts->query('jquery-ui-core');

      wp_register_style('jquery-ui-core', pa::$add_ons['pa']['url'] . '/parts/css/jquery-ui/jquery-ui.css', false, $jquery_ui_core->ver);
      wp_register_style('jquery-ui-core-pa', pa::$add_ons['pa']['url'] . '/parts/css/jquery-ui.pa.css', false, pa::$version);

      wp_enqueue_style('jquery-ui-core');
      wp_enqueue_style('jquery-ui-core-pa');
  ...
  public static function render_field_custom_assets($type)
  {
    switch ($type)
    {
      ...
      default:
         ...
         * @since 1.0
         */
        do_action('pa_render_field_custom_assets', $type);
      ...
      case 'field':
        ...
        else
        {
          ...
          if (((isset($field_rendering_value[0]) && null !== $field_rendering_value[0]) && !self::$field_rendering['multiple'])
              || (self::$field_rendering['multiple'] && !pa::is_flat($field_rendering_value))
          ...
          elseif (is_array($values) && !pa::is_associative($values))
          {
            $clone = self::$field_rendering;
            ...
            for ($index = 0; $index < count($values); $index++)
            {
              ...
              {
                array_push($clone['attributes']['class'], 'pa-error');
              }
          ...
          if (self::$field_rendering['relate'] && self::$field_rendering['type'] != 'group' && in_array(self::$field_rendering['scope'], array('post', 'user', 'comment')))
          {
            ...
            if (!isset(self::$related_object_ids[$context]) || !in_array(self::$field_rendering['object_id'], self::$related_object_ids[$context]))
            {
              ...
              $content .= self::render_field(array(
                ...
                ,'attributes' => array(
                  'class' => 'pa-field-part'
                  ,'data-pa-field-addmore-clear' => true
                )
              ), true);
            }
          }
  ...
  public static function template_label($field)
  {
    ...
    $attributes = array(
      ...
      ,'class' => 'pa-field-part '
                  . 'pa' . ($field['child_field'] ? '-child' : '') . '-label '
                  . 'pa-label-position-' . $field['label_position'] . ' '
                  . ($field['type'] == 'group' ? 'pa-group-label ' : '')
  ...
  {
    return pa::render('fields/' . $type, $field, true);
  }
  ...
  public static function template_field_group($field)
  {
    ...
    foreach ($field['fields'] as &$column)
    {
      ...
      array_push($column['attributes']['wrapper_class'], 'pa-field-part');
      ...
      if ($field['errors'])
      {
        array_push($column['attributes']['class'], 'pa-error');
      }
      ...
      // Relate field
      $column = pa_relate::relate_field($column);
      ...
      if (!$field['field'] && !$column['new'])
      {
        ...
        $cardinality = !$column['multiple'] && count($column_value) > $cardinality && (!is_array($field['value']) || (is_array($field['value']) && !pa::is_associative($field['value']))) ? count($column_value) : $cardinality;
      }
    }
    ...
    if ($field['field'])
    {
      if (empty($field['value']))
      {
        ...
        $cardinality = count($field['value']) > 1 && (!is_array($field['value']) || (is_array($field['value']) && !pa::is_associative($field['value']))) ? count($field['value']) : 1;
      }
    }
    ...
    for ($index = 0; $index < $cardinality; $index++)
    {
      ...
      foreach ($field['fields'] as $column)
      {
        ...
        $column['attributes']['data-pa-field-group'] = $group_id;
        ...
        if ($column['type'] == 'group')
        {
          ...
          {
            $_field['attributes']['data-pa-field-sub-group'] = $group_id;
          }
          unset($_field);
        }

        // Check sortable
        if (isset($field['attributes']['data-pa-field-sortable']))
        {
          $column['attributes']['data-pa-field-sortable'] = $field['attributes']['data-pa-field-sortable'];
        }

        // Check add more
        if ($column['type'] != 'group' && !$group_add_more && isset($field['attributes']['data-pa-field-addmore']))
        {
          $group_add_more = true;

          $column['attributes']['data-pa-field-addmore'] = $field['attributes']['data-pa-field-addmore'];
          $column['attributes']['data-pa-field-addmore-actions'] = $field['attributes']['data-pa-field-addmore-actions'];
        }

        // Check add more type
        if (isset($column['add_more']) && $column['add_more'] && isset($field['attributes']['data-pa-field-addmore']))
        {
          $column['attributes']['data-pa-field-addmore-single'] = $field['attributes']['data-pa-field-addmore'];
        }
        ...
        if (!empty($field['conditions']))
        {
          if (is_array($column['conditions']))
          {
            ...
            array_push($column['attributes']['class'], 'pa-field-condition');
          }
          else
          {
        ...
        if ($column['type'] != 'html')
        {
          ...
          if (pa_validate::errors())
          {
            $column['errors'] = pa_validate::get_errors($column);

            $column['value'] = pa_validate::get_request_value($column);

            // Update cardinality if necessary
            if (!$field['field'])
            {
              $cardinality = !$column['multiple'] && count(is_array($column['value']) ? $column['value'] : []) > $cardinality && (!is_array($field['value']) || (is_array($field['value']) && !pa::is_associative($field['value']))) ? count(is_array($column['value']) ? $column['value'] : []) : $cardinality;
            }

            if (is_array($column['value']) && (!$column['multiple'] || ($column['multiple'] && !pa::is_flat($column['value']))))
            {
          ...
          elseif (is_array($field['value']))
          {
            ...
            $column['value'] = pa::array_path_get($field['value'], $path);
          }
          elseif (!$column['new'])
          {
  ...
  public static function field_label($field)
  {
    ...
    $label .= $field['label'];
    $label .= $field['required'] ? '<span class="pa-required">*</span>' : null;
    $label .= $field['help'] ? pa::render('shared/tooltip-help', array('message' => $field['help']), true) : null;
  ...
  public static function register_forms()
  {
    ...
    pa::process_parts('forms', $data, array('pa_form', 'register_forms_callback'));
  }
  ...
  public static function render_form($form, $add_on = null, $return = false)
  {
    ...
    if (!isset(self::$forms[$form]))
    {
      ...
      if (!is_null($add_on))
      {
        $_form = pa::slug($add_on . ' ' . $form);
      }

      if (!isset(self::$forms[$_form]))
      {
        $_form = pa::slug(pa_add_on::current() . ' ' . $form);
      }
    ...
    {
      $output = pa::render('fields/form', $form, true);
    ...
    {
      _e('You must be logged in to view this form.', 'pa');
    }
  }
  ...
  {
    if (self::valid() && !pa_admin::is_widget() && !pa_admin::is_setting())
    {
      $form_id = isset($_REQUEST[pa::$prefix]['form_id']) ? $_REQUEST[pa::$prefix]['form_id'] : false;
      ...
      if (self::save())
      {
        $redirect = isset($_REQUEST[pa::$prefix]['redirect']) ? $_REQUEST[pa::$prefix]['redirect'] : false;
  ...
  public static function save()
  {
    ...
    $check = pa_validate::check();

    // Get our field data after its been sanitized and validated
    if (!isset($_REQUEST[pa::$prefix]['fields']) || isset($_REQUEST[pa::$prefix]['filter']) || !$check['valid'] || !in_array($check['type'], array('POST', 'PUT')))
    {
    ...
    foreach ($fields_data as $scope => &$fields)
    {
      if (in_array($scope, array('post_meta', 'term_meta', 'user_meta', 'comment_meta')))
      {
        ...
        foreach ($fields as &$field)
        {
          if (!$field['display'] && array_key_exists(pa::$prefix . $scope, $_FILES) && array_key_exists($field['field'], $_FILES[pa::$prefix . $scope]['name']))
          {
            $paths = pa::array_paths($_FILES[pa::$prefix . $scope]['name'][$field['field']]);

            $allowed = $field['role'] === null || $field['capability'] === null || current_user_can('upload_files');

            if (!$allowed)
            {
              $field = pa_validate::add_error($field, 0, __('Insufficient permissions to save this data.', 'pa'));
            }
            ...
            if (!empty($paths) && $allowed)
            {
              if (strstr($paths[0], ':'))
              {
                foreach ($paths as $path)
                {
                  ...
                  if ($upload)
                  {
                    pa::array_path_set($field['request_value'], explode(':', $path), current($upload));
                  }
                }
              }
              else
    ...
    foreach ($fields_data as $scope => &$fields)
    {
            ...
            if ($scope == 'user' && isset($object['signon']))
            {
              ...
              else
              {
                foreach ($fields as &$field)
                {
                  ...
                  {
                    $field = pa_validate::add_error($field, 0, __('Invalid credentials.', 'pa'));
                  }
                }
              }
            }
      ...
      {
        $meta_type = substr($scope, 0, strpos($scope, '_'));
        $meta = pa_meta::get_meta_properties($meta_type);
        ...
        foreach ($fields as &$field)
        {
          ...
          if ($field['object_id'] && !$field['display'] && substr($save_as, 0, strlen('_' . pa::$prefix . 'relate_')) != '_' . pa::$prefix . 'relate_' && !strstr($field['field'], ':') && $allowed)
          {
            ...
            foreach ($meta_object_ids as $index => $meta_object_id)
            {
              ...
              if ($grouped)
              {
                $current_group_meta_ids = $wpdb->get_col($wpdb->prepare("SELECT $meta->id FROM $meta->table WHERE $meta->object_id = %d AND meta_key = %s", $meta_object_id, '_' . pa::$prefix . $save_as));
              }
                ...
                foreach ($request_value as $values)
                {
                  if (is_array($values))
                  {
                    ...
                    if ($grouped)
                    {
                      ...
                      {
                        add_metadata($meta_type, $meta_object_id, '_' . pa::$prefix . $save_as, $meta_ids);
                      }
                    }
        ...
        foreach ($fields as &$field)
        {
          ...
          if ($allowed)
          {
            if ($field['field'] && !stristr($field['field'], ':'))
            {
              ...
              if (is_array($value) && pa::is_flat($value) && count($value) == 1)
              {
                $value = current($value);
              }
       ...
       * @since 1.0
       */
      do_action('pa_save_field', $scope, $fields);
  ...
  public static function save_upload($path, $storage = array(), $url = false)
  {
    ...
    $codes = pa::array_path_get($files, $paths['error']);
    $names = pa::array_path_get($files, $paths['name']);
    $sizes = pa::array_path_get($files, $paths['size']);
    $tmp_names = pa::array_path_get($files, $paths['tmp_name']);
    ...
    switch ($type)
    {
      ...
      case 'user':
        ...
        if (isset($id) && !is_wp_error($id))
        {
          ...
          {
            pa_user::multiple_roles($id, $object['user_role']);
          }
        }
  ...
  public static function relate()
  {
    ...
    foreach (self::$form_submission as $fields)
    {
      foreach ($fields as $field)
      {
        if ($field['relate_to'] && $field['field'])
        {
          ...
          $is_meta = in_array($field['scope'], array('post_meta', 'user_meta', 'comment_meta')) && substr($field['field'], 0, strlen('_' . pa::$prefix . 'relate_')) == '_' . pa::$prefix . 'relate_';
    ...
    foreach ($related as $to => $objects)
    {
      ...
      {
        $meta_key = '_' . pa::$prefix . 'relate_' . $object['scope_from'];
  ...
  {
    $prefix = pa::$prefix . 'post_post_content';
    ...
    if ($editor_id != 'content' && substr($editor_id, 0, strlen($prefix)) !== $prefix)
    {
      ...
      array_push($content_css,  pa::$add_ons['pa']['url'] . '/parts/css/tinymce-pa.css');
  ...
  /**
   * notices
   * Handle any form notices for forms with pa fields.
   *
