...
class Pa_Shortcode
{
  ...
  {
    add_action('init', array('pa_shortcode', 'register_shortcodes'));
    add_action('admin_init', array('pa_shortcode', 'admin_init'));
    add_action('media_buttons', array('pa_shortcode', 'media_buttons'), 100);
    add_action('print_media_templates', array('pa_shortcode', 'print_media_templates'));
    add_action('wp_editor_settings', array('pa_shortcode', 'wp_editor_settings'), 10, 2);
    add_action('wp_ajax_pa_shortcode', array('pa_shortcode', 'ajax'));
    add_action('wp_ajax_nopriv_pa_shortcode', array('pa_shortcode', 'ajax'));

    add_filter('admin_body_class', array('pa_shortcode', 'admin_body_class'));
    add_filter('pa_part_id-shortcodes', array('pa_shortcode', 'part_id'), 10, 4);
    add_filter('pa_part_process-shortcodes', array('pa_shortcode', 'part_process'), 10, 2);
    add_filter('pa_admin_pages', array('pa_shortcode', 'admin_pages'));
    add_filter('pa_field_templates', array('pa_shortcode', 'field_templates'));
    add_filter('pa_assets_localize', array('pa_shortcode', 'assets_localize'));
    add_filter('pa_pre_render_field_shortcode_data_content', array('pa_shortcode', 'set_content_field'));
  }
  ...
  {
    add_filter('mce_buttons', array('pa_shortcode', 'mce_buttons'));
    add_filter('tiny_mce_before_init', array('pa_shortcode', 'tiny_mce_before_init'));
    add_filter('mce_external_plugins', array('pa_shortcode', 'mce_external_plugins'), 100);
  }
  ...
  {
    $pages[] = array(
      'page_title' => __('Shortcode Editor', 'pa')
      ,'menu_title' =>__('Shortcode Editor', 'pa')
      ,'capability' => 'read'
      ,'sub_menu' => ''
  ...
  /**
   * part_process
   * Show Shortcode settings if non-Pa shortcodes are active.
   *
  ...
  {
    $shortcodes = pa_shortcode::get_shortcodes();
  ...
  public static function register_shortcodes()
  {
    ...
    pa::process_parts('shortcodes', $data, array('pa_shortcode', 'register_shortcodes_callback'));
  }
  ...
  public static function register_shortcodes_callback($arguments)
  {
    ...
    if (!empty($data['shortcode']))
    {
      ...
      {
        $data['icon'] = pa::$add_ons[$add_on]['url'] . '/' . $data['icon'];
      }
      ...
      foreach ($render as $file)
      {
        ...
        {
          add_shortcode($data['shortcode'], array('pa_shortcode', 'shortcode'));
  ...
  public static function shortcode($attributes, $content, $tag)
  {
    ...
    if (self::$shortcodes[$tag])
    {
      ...
      do_action('pa_pre_render_shortcode', $attributes, self::$shortcodes[$tag]);
      ...
      if (self::$shortcodes[$tag]['render'])
      {
        ...
        foreach (self::$shortcodes[$tag]['render'] as $render)
        {
          ...
          {
            pa::render($render, $attributes);
          }
        }
      }

      do_action('pa_post_render_shortcode', $attributes, self::$shortcodes[$tag]);
  ...
  public static function media_buttons($editor_id)
  {
    ...
    {
      echo '<button type="button" id="pa-shortcode-button" class="button pa-shortcode-button" title="' . __('Add Shortcode', 'pa') . '" data-editor="' . $editor_id . '"><span class="wp-media-buttons-icon"></span> ' . __('Add Shortcode', 'pa') . '</button>';
    }
  }
  ...
  public static function mce_external_plugins($plugins)
  {
    ...
    {
      $plugins['pa_shortcode'] = pa::$add_ons['pa']['url'] . '/parts/js/tinymce-shortcode.js';
    }
  ...
  public static function tiny_mce_before_init($mce_init)
  {
    if (in_array(self::$editor_current, self::$shortcode_editors))
    {
      $content_css = pa::$add_ons['pa']['url'] . '/parts/css/tinymce-shortcode.css';
  ...
  public static function get_shortcodes()
  {
    ...
    /**
     * pa_shortcodes_blacklist
     *
     * @since 1.0
     */
    self::$shortcodes_blacklist = apply_filters('pa_shortcodes_blacklist', self::$shortcodes_blacklist);
    ...
    foreach ($shortcode_tags as $tag => $function)
    {
      ...
      {
        $shortcodes[$tag] = pa::humanize($tag);
      }
    }
  ...
  public static function assets_localize($localize)
  {
    ...
    $shortcodes = pa::get_settings('pa_core', 'shortcode_ui');
      ...
      foreach ($shortcodes as $data)
      {
        ...
        { 
          $localize['shortcodes'][$shortcode] = array(
            'name' => pa::humanize($shortcode)
            ,'description' => __('Click on this box to edit the shortcode properties.', 'pa')
            ,'shortcode' => $shortcode
  ...
  {
    pa::render('shared/template-pa-shortcode');
    pa::render('shared/template-pa-shortcode-inline');
  }
  ...
  public static function ajax()
  {
    ...
    do_action('pa_pre_render_ajax_shortcode', $post_id, $shortcode);

    echo do_shortcode($shortcode);

    do_action('pa_post_render_ajax_shortcode', $post_id, $shortcode);
  ...
  {
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'shortcode_editor' && isset($_REQUEST[pa::$prefix . 'shortcode_data']['name']) && isset(self::$shortcodes[$_REQUEST[pa::$prefix . 'shortcode_data']['name']]))
    {
      $classes .= ' shortcode_editor-' . esc_attr($_REQUEST[pa::$prefix . 'shortcode_data']['name']);
    }
