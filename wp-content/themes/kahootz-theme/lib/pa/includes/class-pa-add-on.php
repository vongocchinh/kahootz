...
/**
 * Pa_Add_On
 * Loads and manages any Pa plugins or add-ons included within Pa plugin.
 *
 * @package     Pa
 * @subpackage  Add ons
...
{
  /**
   * @var string Available pa add-ons and their locations.
   * @access public
   */
  ...
  {
    add_action('init', array('pa_add_on', 'include_add_ons'), 0);
  }

  /**
   * include_add_ons
   * Inlcude add-ons registered with pa.
   *
   * @access public
   * @static
   * @since 1.0
  ...
  public static function include_add_ons()
  {
    ...
    foreach ($plugins as $plugin)
    {
      ...
      if (file_exists($path))
      {
        $data = pa::get_file_data($path, array(
                   'name' => 'Plugin Name'
                  ,'type' => 'Plugin Type'
                  ,'version' => 'Version'
                ));

        if ($data['type'] && strtolower($data['type']) == 'pa')
        {
          pa::add_plugin(basename(dirname($plugin)), dirname($path));

          add_action('load-plugins.php', array('pa_admin', 'deactivation_link'));

          pa_admin::$pa_dependent['plugins'][] = $data;
          ...
          if ($data['version'])
          {
            ...
            pa_admin::check_update($file, $version);
          }
        }
      }
    }

    $addon_paths = pa::paths();
    $paths = array();
    ...
    foreach ($paths as $path)
    {
      if (is_dir($path))
      {
        ...
        {
          $add_ons = pa::get_directory_list($path);
  ...
  private static function register_add_on($add_on, $file, $path, $plugin = false)
  {
    if (file_exists($file))
    {      
      $data = pa::get_file_data($file, array(
                'name' => 'Plugin Name'
      ...
      if (self::is_active($add_on))
      {
        ...
        $class_name = str_replace(pa::$prefix, 'pa_', pa::slug($add_on));
        ...
        pa::$paths[$add_on] = $path . (!$plugin ? '/' . $add_on : '');
        pa::$add_ons[$add_on]['path'] = $path . (!$plugin ? '/' . $add_on : '');

        $path = str_replace(chr(92), '/', str_replace('/add-ons', '', $path));

        pa::$urls[$add_on] = plugins_url() . substr($path, strrpos($path, '/')) . '/add-ons/' . $add_on;
        pa::$add_ons[$add_on]['url'] = plugins_url() . substr($path, strrpos($path, '/')) . '/add-ons/' . $add_on;
      }
    }
  }
  ...
  {
    $add_ons = get_option('pa_core_addons');
