...
class Pa_Revision
{
  ...
  {
    add_action('save_post', array('pa_revision', 'save_post'), 10, 2);
    add_action('wp_restore_post_revision', array('pa_revision', 'restore_revision'), 10, 2);

    add_filter('_wp_post_revision_fields', array('pa_revision', 'wp_post_revision_fields'), 10, 2);
  }
  ...
  public static function save_post($post_id, $post)
  {
    ...
    if (($parent_id = wp_is_post_revision($post_id)) && !wp_is_post_autosave($post_id))
    {
      ...
      $black_listed_keys = apply_filters('pa_revision_meta_blacklist', array(
        '_edit_lock',
        '_edit_last'
      ), $post);
  ...
  public static function wp_post_revision_fields($fields, $post = array())
  {
    ...
    foreach ($meta_keys as $meta_key)
    {
      ...
      $fields[$meta_key] = __($label, 'pa');

      add_filter('_wp_post_revision_field_' . $meta_key, array('pa_revision', 'wp_post_revision_field'), 10, 4);
    }
