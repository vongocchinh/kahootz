...
class Pa_Workflow
{
  ...
  {
    add_filter('redirect_post_location', array('pa_workflow', 'redirect'), 10, 2);
    add_filter('wp_redirect', array('pa_workflow', 'redirect'), 10, 2);
    add_filter('pa_part_process_callback', array('pa_workflow', 'part_process_callback'), 100, 2);
    add_filter('pa_part_data', array('pa_workflow', 'part_data'), 10, 2);
    add_filter('admin_body_class', array('pa_workflow', 'admin_body_class'));
    add_filter('pa_validate_part_parameter_skip', array('pa_workflow', 'validate_part_parameter_skip'), 10, 3);
    add_action('init', array('pa_workflow', 'register_workflows'), 100);
    add_action('pa_parts_processed-workflows', array('pa_workflow', 'detect_workflow'), 100);

    add_shortcode('pa_workflow', array('pa_workflow', 'shortcode'));
  }
  ...
  public static function redirect($location, $post_id)
  {
    if (isset($_REQUEST['_wp_http_referer']))
    {
      ...
      if (isset($url['query']))
      {
        ...
        if ((isset($url_defaults[pa::$prefix]['flow']) && !stristr($location, 'flow=')) && (isset($url_defaults[pa::$prefix]['flow_page']) && !stristr($location, 'flow_page=')))
        {
          $url_arguments = array(
            pa::$prefix => array(
              'flow' => urlencode($url_defaults[pa::$prefix]['flow'])
              ,'flow_page' => urlencode($url_defaults[pa::$prefix]['flow_page'])
            )
          );
  ...
  public static function register_workflows()
  {
    ...
    pa::process_parts('workflows', $data, array('pa_workflow', 'register_workflows_callback'));

    self::$after_positions = array(
      'header' => 'in_admin_header'
      ,'body' => 'all_admin_notices'
      ,'title' => pa_admin::is_post() ? 'edit_form_after_title' : 'pa_admin_page_after_title'
      ,'editor' => 'edit_form_after_editor'
    );

    foreach (self::$after_positions as $position => $filter)
    {
      add_action($filter, array('pa_workflow', 'render_workflow'));
    }
  }
  ...
  public static function register_workflows_callback($arguments)
  {
    ...
    foreach ($data['flow'] as $flow)
    {
      ...
      $data['flow_page'] = pa::slug($tab ? $tab : $data['title'], 'UTF-8');
      $data['flow_slug'] = $flow == pa::dashes($flow) ? $flow : pa::slug($flow, 'UTF-8');
      $data['flow_sub_page'] = $tab ? pa::slug($data['title'], 'UTF-8') : false;

      if ((pa_admin::is_user() || pa_admin::is_term()) && $data['position'] == 'title')
      {
        $data['position'] = 'body';
      }
      ...
      {
        /**
         * pa_workflow_redirect_url
         *
         * @since 1.0
         */
        $data['redirect'] = apply_filters('pa_workflow_redirect_url', $data['redirect'], $workflow, $data);

        $url_query_string = http_build_query(array_filter($url));

        $workflow['url'] = is_admin() ? admin_url($data['redirect'] . (strstr($data['redirect'], '?') ? '&' : '?') . $url_query_string) : pa::current_url() . '?' . $url_query_string;
      }
      elseif ($data['disable'] == false)
      {
        ...
        $workflow['url'] = is_admin() ? admin_url($pagenow . '?' . $url_query_string) : pa::current_url() . '?' . $url_query_string;
      }
  ...
  public static function detect_workflow($folder = null, $shortcode = false)
  {
    ...
    {
      uasort($workflows, array('pa', 'sort_by_data_order'));
      ...
      if (self::is_allowed($has_data) || $shortcode)
      {
        ...
        foreach ($workflows as $index => &$workflow)
        {
          ...
          else
          {
            ...
            if ($workflow['data']['active'])
            {
              ...
              $tab = pa::slug($workflow['data']['title'], 'UTF-8');
              $data = $workflow['data'];
            }
            ...
            if (isset(self::$sub_workflows[$flow][$tab]))
            {
              ...
              uasort($workflow['parts'], array('pa', 'sort_by_data_order'));
              ...
              foreach ($workflow['parts'] as $sub_index => &$sub_workflow)
              {
                ...
                else
                {
                  ...
                  if ($sub_workflow['data']['active'])
                  {
                    ...
                    $sub_tab = pa::slug($sub_workflow['data']['title'], 'UTF-8');
                    $data = $workflow['data'];
                  }
              ...
              if ($workflow['data']['active'] && is_null($sub_tab))
              {
                ...
                $sub_tab = !empty($default_sub_workflow['data']['title']) ? pa::slug($default_sub_workflow['data']['title'], 'UTF-8') : null;
        ...
        if (!$tab)
        {
          if(!empty($default_workflow))
          {
            ...
            $tab = pa::slug($default_workflow['data']['title'], 'UTF-8');
        ...
        if (isset($data)) 
        {
          // Initialize self::$workflow as array if it's false
          if (self::$workflow === false) {
            self::$workflow = array();
          }
  ...
  public static function is_allowed($data, $strict = true)
  {
    ...
    if (isset($_REQUEST['flow']) && pa::slug($flow, 'UTF-8') == $_REQUEST['flow'])
    {
      $allowed = true;
    }
	...
	if (!empty($data))
	{
		...
		// Check Post Types
		if ($allowed && pa_admin::is_post())
		{
			$post_types = $data['data']['post_type'];
			$current_post_type = pa_cpt::detect_post_type();
  ...
  public static function is_active($data)
  {
    ...
    if ($tab && isset($_REQUEST[pa::$prefix]['flow_sub_page']))
    {
      $is_active = esc_attr($_REQUEST[pa::$prefix]['flow_page']) === $data['flow_page'] && esc_attr($_REQUEST[pa::$prefix]['flow_sub_page']) === $data['flow_sub_page'];
    }
    elseif (!$tab && isset($_REQUEST[pa::$prefix]['flow_page']))
    {
      $is_active = esc_attr($_REQUEST[pa::$prefix]['flow_page']) === $data['flow_page'];
    }
    else
    {
  ...
  public static function render_workflow()
  {
    if (self::$workflow)
    {
      ...
      if (self::$rendering)
      {
        pa::render(is_admin() ? 'shared/admin-workflow' : 'shared/workflow', self::$workflow);
  ...
  {
    /**
     * pa_workflow_part_exclude_folders
     *
     * @since 1.0
     */
    $exclude_folders = apply_filters('pa_workflow_part_exclude_folders', array('notices', 'pointers', 'help', 'widgets', 'shortcodes'), $part, $folder);
  ...
  public static function admin_body_class($classes)
  {
    if (self::$workflow)
    {
      $classes .= ' pa-workflow-active pa-workflow-' . self::$workflow['flow'] . ' pa-workflow-tab-' . self::$workflow['tab'];

      if (self::$workflow['sub_tab'])
      {
        $classes .= ' pa-workflow-sub-tab-' . self::$workflow['tab'];
      }
    }
