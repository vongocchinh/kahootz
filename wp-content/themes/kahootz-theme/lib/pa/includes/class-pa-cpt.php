...
{
  /**
   * @var array Post Types registered with pa_post_types
   * @access private
   */
  private static $post_types = array();

  /**
   * @var array Taxonomies registered with pa_taxonomies
   * @access private
   */
  ...
  {
    add_action('init', array('pa_cpt', 'init'));
    add_action('pre_get_posts', array('pa_cpt', 'pre_get_posts'), 100);
    add_action('edit_page_form', array('pa_cpt', 'edit_form'));
    add_action('edit_form_advanced', array('pa_cpt', 'edit_form'));
    add_action('restrict_manage_posts', array('pa_cpt', 'taxonomy_filters_list_table'));
    add_action('admin_footer', array('pa_cpt', 'quick_edit_post_statuses'));
    add_action('pa_save_fields', array('pa_cpt', 'save_fields'));

    add_filter('post_row_actions', array('pa_cpt', 'post_row_actions'), 10, 2);
    add_filter('page_row_actions', array('pa_cpt', 'post_row_actions'), 10, 2);
    add_filter('wp_insert_post_data', array('pa_cpt', 'wp_insert_post_data'), 100, 2);
    add_filter('display_post_states', array('pa_cpt', 'display_post_states'), 999);
    add_filter('pa_assets_localize', array('pa_cpt', 'assets_localize'));

    if (pa_admin::is_post())
    {
      add_filter('is_protected_meta', array('pa_cpt', 'is_protected_meta'), 100, 3);
    }
  }
  ...
  /**
   * edit_form
   * Appends pa variables to the edit form if necessary.
   *
   * @access public
   * @static
   * @since 1.0
  ...
  public static function edit_form()
  {
    ...
    {
      if (isset($_REQUEST[pa::$prefix][$field]) && !empty($_REQUEST[pa::$prefix][$field]))
      {
        pa_form::render_field(array(
          'type' => 'hidden'
          ,'scope' => pa::$prefix
          ,'field' => $field
          ,'value' => $_REQUEST[pa::$prefix][$field]
        ));
      }
    }
  }
  ...
  /**
   * register_post_types
   * registers Post Types that use the pa_post_types filter.
   *
   * @access public
   * @static
   * @since 1.0
  ...
  public static function register_post_types()
  {
    /**
     * pa_post_types
     * Register Post Types with Pa
     *
     * Allows for all custom Pa parameters when registering a Post Type
     *
     * @param array $post_types
     *
     * @since 1.0
     */
    self::$post_types = apply_filters('pa_post_types', self::$post_types);
    ...
    foreach (self::$post_types as $post_type => &$configuration)
    {
      ...
      {
        /**
         * pa_post_type_statuses
         *
         * @since 1.0
         */
        $configuration['status'] = apply_filters('pa_post_type_statuses', $configuration['status'], $post_type);
      ...
      add_action('admin_head', array('pa_cpt', 'hide_meta_boxes'), 100);

      if (!empty($configuration['title']))
      {
        add_filter('enter_title_here', array('pa_cpt', 'enter_title_here'));
      }
      ...
      {
        add_filter('screen_options_show_screen', array('pa_cpt', 'hide_screen_options'));
      }

      if (!empty($configuration['edit_columns']))
      {
        add_filter('manage_edit-' . $post_type . '_columns', array('pa_cpt', 'manage_edit_columns'));
      }

      if (!empty($configuration['admin_body_class']))
      {
        add_filter('admin_body_class', array('pa_cpt', 'admin_body_class'), 10000);
      }

      add_filter('post_updated_messages', array('pa_cpt', 'post_updated_messages_filter'));

    }

    self::sort_post_statuses();

    self::flush_rewrite_rules(md5(serialize($check)), 'pa_post_type_rules_flushed');
  }

  /**
   * register_taxonomies
   * registers Taxonomies that use the pa_taxonomies filter.
   *
   * @access public
   * @static
   * @since 1.0
  ...
  public static function register_taxonomies()
  {
    ...
    /**
     * Register Taxonomies with Pa
     *
     * Allows for all custom Pa parameters when registering a Taxonomy
     *
     * @param array $taxonomies
     *
     * @since 1.0
     */
    self::$taxonomies = apply_filters('pa_taxonomies', self::$taxonomies);
    ...
    foreach (self::$taxonomies as $taxonomy_name => $taxonomy)
    {
      ...
      if (isset($taxonomy['object_type']) && $taxonomy['object_type'] == 'user')
      {
        ...
        {
          pa_cpt::$taxonomy = $taxonomy;

          add_filter('manage_users_columns', array('pa_cpt', 'user_column_header'), 10);
          add_action('manage_users_custom_column', array('pa_cpt', 'user_column_data'), 10, 3);
        }
      }
      ...
      if (isset($taxonomy['configuration']['meta_box_filter']) && $taxonomy['configuration']['meta_box_filter'] == '1')
      {
        if (in_array($pagenow, array('post.php', 'post-new.php')))
        {
          ...
          add_action('admin_head', array('pa_cpt', 'taxonomy_filter_meta_box'));
        }
      }
    }

    self::flush_rewrite_rules(md5(serialize($check)), 'pa_taxonomy_rules_flushed');
  }
  ...
  {
    foreach (pa_cpt::$taxonomy_filter_meta_box as $taxonomy)
    {
      pa::render('shared/taxonomy-filter-meta-box', array(
        'name' => $taxonomy['name']
      ));
    }
  }
  ...
  {
    $columns[pa_cpt::$taxonomy['name']] = pa_cpt::$taxonomy['configuration']['labels']['name'];
  ...
  {
    $term = wp_get_object_terms($value, pa_cpt::$taxonomy['name']);
  ...
  public static function quick_edit_post_statuses($column_name)
  {
    ...
    if ($pagenow == 'edit.php')
    {
      ...
      if (!empty($post_statuses))
      {
        ...
        pa::render('shared/list-table-post-statuses', array(
          'choices' => $choices
        ));
      }
    }
  ...
  public static function wp_insert_post_data($data, $post_array)
  {
    if (($data['post_status'] != 'auto-draft') && (($data['post_title'] == __('Auto Draft') || empty($data['post_title']))))
    {
       ...
       * @since 1.0
       */
      $data['post_title'] = apply_filters('pa_empty_post_title', $data['post_title'], $post_array);
    }
  ...
  {
    if ($query->is_main_query() && isset($_REQUEST) && (isset($_REQUEST[pa::$prefix]['filter']) && strtolower($_REQUEST[pa::$prefix]['filter']) == 'true') && isset($_REQUEST[pa::$prefix]['fields']))
    {
      $args = array(
        'meta_query' => array()
        ,'tax_query' => array()
      ...
      $fields = get_transient(pa::$prefix . $_REQUEST[pa::$prefix]['fields']);
      ...
      foreach (self::$search_data as $key => $values)
      {
        foreach ($values as $_key => $_values)
        {
          ...
          {
            $filter = substr($key, 0, 1) == pa::$prefix ? substr($key, strlen(pa::$prefix)) : $key;
      ...
      if (isset(self::$search_data[pa::$prefix . 'post']))
      {
        add_filter('posts_search', array('pa_cpt', 'posts_search'), 10, 2);
      }
    }
  }
  ...
  public static function posts_search($search, &$query)
  {
    ...
    if ($query->is_main_query() && !empty(self::$search_data[pa::$prefix . 'post']) && empty($query->query_vars['s']))
    {
      $n = !empty($query->query_vars['exact']) ? '' : '%';
      foreach (self::$search_data[pa::$prefix . 'post'] as $field => $terms)
      {
        $search_terms = '';
        $search_or = '';
		...
		if (in_array($pagenow, array('edit.php', 'upload.php')))
    {
      ...
      foreach (self::$taxonomies as $taxonomy)
      {
        if (in_array($taxonomy['name'], $taxonomies))
        {
          ...
          {
						pa::render('shared/list-table-filter-taxonomies', array(
  ...
  public static function save_fields($fields)
  {
    if (array_key_exists('post_meta', $fields))
    {
      ...
      if (!empty($meta_keys))
      {
        $saved_meta_keys = get_option('pa_post_meta_keys');
        ...
        update_option('pa_post_meta_keys', $saved_meta_keys, true);
      }
    }
  }
  ...
  {
    $saved_meta_keys = get_option('pa_post_meta_keys', null);
