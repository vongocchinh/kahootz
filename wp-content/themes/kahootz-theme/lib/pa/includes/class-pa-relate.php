...
class Pa_Relate
{
  ...
  public static function register_meta()
  {
    ...
    foreach($object_types as $object_type)
    {
      ...
      {
        register_meta($object_type, '_' . pa::$prefix . "relate_$type", array(__CLASS__, 'absint_callback'), '__return_true');
      }
    }
  }
    ...
    foreach($relations as $relation) {
      ...
      elseif(!(empty($relation['relate']) || empty($relation['id'])))
      {
        ...
        if ('has' === strtolower($relation['relate']))
        {
          ...
          $new_ids = get_metadata($scope, $relation['id'], '_' . pa::$prefix . 'relate_' . $main_scope);
        }
        else
        {
  ...
  public static function get_object_ids($field, $is_query = false)
  {
    ...
    $meta_key = '_' . pa::$prefix . 'relate_' . $scope;
    ...
    switch ($field['relate']['scope'])
    {
      ...
      case 'post_meta':
        ...
        if (isset($field['relate']['query']) && $object_ids)
        {
          ...
          $object_ids = pa($query->posts, array('ID'));
        }
      ...
      case 'user_meta':
        ...
        if (isset($field['relate']['query']) && $object_ids)
        {
          ...
          $object_ids = pa($query->results, array('ID'));
        }
  ...
  public static function relate_field($field)
  {
    ...
    if (!isset($field['relate']['field']))
    {
      switch ($field['relate']['scope'])
      {
        ...
        case 'post_meta':
          ...
          $field['relate_to'] = pa_admin::is_post();
        ...
        case 'user_meta':
          ...
          $field['relate_to'] = pa_admin::is_user();
        ...
        case 'comment_meta':
          ...
          $field['relate_to'] = pa_admin::is_comment();
    ...
    if (substr($field['field'], 0, strlen('_' . pa::$prefix . 'relate_')) == '_' . pa::$prefix . 'relate_')
    {
  ...
  public static function relate_objects($owner, $has)
  {
    ...
    $key = '_' . pa::$prefix . 'relate_' . $owner['scope'];
  ...
  public static function unrelate_objects($owner, $has)
  {
    ...
    $key = '_' . pa::$prefix . 'relate_' . strtolower($owner['scope']);
