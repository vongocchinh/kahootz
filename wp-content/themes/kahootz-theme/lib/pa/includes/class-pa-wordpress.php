...
class Pa_WordPress
{
  ...
  {
    add_action('pre_user_query', array('pa_wordpress', 'pre_user_query'));
    add_action('posts_where', array('pa_wordpress', 'relation_taxonomy'));
    add_action('wp_scheduled_delete', array('pa_wordpress', 'garbage_collection'));
    add_action('pre_get_posts', array('pa_wordpress', 'pre_get_posts'));
    add_action('pa_pre_render_workflow', array('pa_wordpress', 'pre_render_workflow'));

    add_filter('pa_part_data', array('pa_wordpress', 'part_data'), 10, 2);
    add_filter('pa_part_data_parameter', array('pa_wordpress', 'pa_part_data_parameter'), 10, 2);
    add_filter('get_meta_sql', array('pa_wordpress', 'get_meta_sql'), 101, 6);
  }
  ...
  public static function get_meta_sql($sql, $query, $type, $primary_table, $primary_id_column, $context, $parent_relation = null, $depth = 0)
  {
     ...
     * @since 1.0
     */
    if (pa::get_settings('pa_core', 'meta_queries') !== 'true' || !apply_filters('pa_get_meta_sql', true, $sql, $query, $type, $primary_table, $primary_id_column, $context, $parent_relation, $depth))
    {
      return $sql;
    }
  ...
  public static function pre_user_query(&$query)
  {
    ...
    if (isset($query->query_vars['roles']) && is_array($query->query_vars['roles']))
    {
      ...
      remove_action('pre_user_query', array('pa_wordpress', 'pre_user_query'));
        ...
        if (self::is_first_order_clause($clause))
        {
          ...
          switch (self::$primary_type)
          {
            case 'post':
              ...
              add_filter('posts_where', array('pa_wordpress', 'where'), 10, 2);
              add_filter('posts_where_request', array('pa_wordpress', 'where'), 10, 2);
              add_filter('posts_orderby', array('pa_wordpress', 'orderby'), 10, 2);
              add_filter('posts_orderby_request', array('pa_wordpress', 'orderby'), 10, 2);
      ...
      if ($depth == 0)
      {
        switch (self::$primary_type)
        {
          case 'post':
            ...
            add_filter('posts_distinct', array('pa_wordpress', 'distinct'), 10, 2);
            add_filter('posts_distinct_request', array('pa_wordpress', 'distinct'), 10, 2);
  ...
  {
    remove_filter('posts_where', array('pa_wordpress', 'where'), 10);
    remove_filter('posts_where_request', array('pa_wordpress', 'where'), 10);
  ...
  {
    remove_filter('posts_orderby', array('pa_wordpress', 'orderby'), 10);
    remove_filter('posts_orderby_request', array('pa_wordpress', 'orderby'), 10);
  ...
  {
    remove_filter('posts_distinct', array('pa_wordpress', 'distinct'), 10);
    remove_filter('posts_distinct_request', array('pa_wordpress', 'distinct'), 10);
  ...
  public static function pre_render_workflow($active_tab)
  {
    ...
    if (isset($active_tab['data']['default_form']) && !$active_tab['data']['default_form'])
    {
      ...
      if ($type)
      {
        pa::render('shared/wordpress-form-hide', array(
          'type' => $type
        ));
      }
    }
   ...
   * @since 1.0
   */
  public static function pa_part_data_parameter($value, $parameter)
  {
