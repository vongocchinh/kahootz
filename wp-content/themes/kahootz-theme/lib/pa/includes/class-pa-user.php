...
class Pa_User
{
  ...
  {
    add_action('init', array('pa_user', 'init'));
    add_action('show_user_profile', array('pa_user', 'meta_box'));
    add_action('edit_user_profile', array('pa_user', 'meta_box'));
  }
  ...
  public static function init()
  {
    ...
    $use_multiple_user_roles = pa::get_settings('pa_core', 'multiple_user_roles');

    if ($use_multiple_user_roles && (!is_multisite() || (isset($pagenow) && $pagenow == 'user-edit.php' && is_multisite())))
    {
      add_action('profile_update', array('pa_user', 'multiple_roles'));
      add_action('user_register', array('pa_user', 'multiple_roles'), 9);
      add_action('admin_footer', array('pa_user', 'multiple_roles_field'));

      add_filter('additional_capabilities_display', array('pa_user', 'additional_capabilities_display'));
    }
  }
  ...
  public static function register_meta_boxes()
  {
    ...
    pa::process_parts('users', $data, array('pa_user', 'register_meta_boxes_callback'));
  }
  ...
  public static function meta_box($user_id)
  {
    if (!empty(self::$meta_boxes))
    {
      ...
      uasort(self::$meta_boxes, array('pa', 'sort_by_data_order'));

      foreach (self::$meta_boxes as $meta_box)
      {
        pa::render('shared/meta-box-start', array(
          'meta_box' => $meta_box
          ,'wrapper' => 'user_meta'
        ), false);

        do_action('pa_pre_render_user_meta_box', $user, $meta_box);
        ...
        do_action('pa_post_render_user_meta_box', $user, $meta_box);
        
        pa::render('shared/meta-box-end', array(
          'meta_box' => $meta_box
          ,'wrapper' => 'user_meta'
        ), false);
      }
  ...
  public static function multiple_roles_field()
  {
    ...
    if (in_array($pagenow, array('user-edit.php', 'user-new.php')))
    {
      ...
      pa::render('shared/field-user-role', array(
  ...
  public static function available_capabilities()
  {
    ...
    $caps = pa($roles, array('capabilities'));
