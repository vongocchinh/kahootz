...
class Pa_Widget
{
   ...
   * @access private
   */
  private static $widget_class = 'pa_universal_widget';
  ...
  {
    add_action('init', array('pa_widget', 'init'));
    add_action('widgets_init', array('pa_widget', 'register_widgets'));
    add_action('widgets_init', array('pa_widget', 'register_widget_groups'), 90);

    add_filter('dynamic_sidebar_params', array('pa_widget', 'dynamic_sidebar_params'));
  }
  ...
  /**
   * register_sidebars
   * Register sidebars via the pa_sidebars
  ...
  public static function register_sidebars()
  {
     ...
     * @since 1.0
     */
    $sidebars = apply_filters('pa_sidebars', array());
  ...
  public static function register_widgets()
  {
    ...
    pa::process_parts('widgets', $data, array('pa_widget', 'register_widgets_callback'));

    // Create widget groups 
    $addons_paths = pa::paths();
    
    foreach ($addons_paths as $from => $path)
    {
      if (!pa::directory_empty($path . '/parts/widgets'))
      {
        $widget_class_name = self::$widget_class . '_' . pa::slug($from);

        $suffix = '';
        $title = '';
        $description = '';

        if (isset(pa_add_on::$available_add_ons[$from]))
        {
          if (stripos(pa_add_on::$available_add_ons[$from]['name'], 'widget') === false)
          {
            $suffix = ' ' . __('Widgets', 'pa');
          }

          $title = pa_add_on::$available_add_ons[$from]['name'] . $suffix;

          $description = strip_tags(pa_add_on::$available_add_ons[$from]['description']);
        }
        elseif ($from == 'pa')
        {
          $title = __('Pa Widgets', 'pa');
          $description = __('Core Widgets for Pa.', 'pa');
        }
        elseif ($from == 'theme')
        {
          $current_theme = wp_get_theme();

          $title = $current_theme . ' ' . __('Widgets', 'pa');
          $description = sprintf(__('Widgets for the %s Theme', 'pa'), $current_theme);
        }

        /**
         * pa_widget_groups_customize_selective_refresh
         ...
         * @since 1.0
         */
        $customize_selective_refresh = apply_filters('pa_widget_groups_customize_selective_refresh', true, $from);
