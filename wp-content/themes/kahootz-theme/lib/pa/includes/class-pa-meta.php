...
/**
 * Pa_Meta
 * Handles all aspects of meta data for Pa
 *
 * @package     Pa
 * @subpackage  Meta
...
class Pa_Meta
{
  ...
  /**
   * @var array Non Pa meta-boxes that have already been removed.
   * @access private
   */
  ...
  {
    add_action('init', array('pa_meta', 'meta_grouped'), 100);
    add_action('init', array('pa_meta', 'meta_reset'));
    add_action('query', array('pa_meta', 'meta_sort'));
    add_action('add_meta_boxes', array('pa_meta', 'register_meta_boxes'), 1000);
    add_action('admin_head', array('pa_meta', 'sort_meta_boxes'), 1050, 3);
    add_action('pa_parts_process-meta-boxes', array('pa_meta', 'clear_screen'), 50);

    add_filter('get_post_metadata', array('pa_meta', 'get_post_meta'), 100, 4);
    add_filter('get_user_metadata', array('pa_meta', 'get_user_meta'), 100, 4);
    add_filter('get_term_metadata', array('pa_meta', 'get_term_meta'), 100, 4);

    add_filter('wp_save_post_revision_check_for_changes', array('pa_meta', 'wp_save_post_revision_check_for_changes'), -1, 3);
    add_filter('wp_save_post_revision_post_has_changed', array('pa_meta', 'wp_save_post_revision_post_has_changed'), -1, 3);
    add_filter('get_post_metadata', array('pa_meta', 'wp_save_post_revision_post_meta_serialize'), 100, 4);

    add_filter('pa_part_process-meta-boxes', array('pa_meta', 'part_process'), 10, 2);
  }
  ...
  public static function register_meta_boxes()
  {
    ...
    pa::process_parts('meta-boxes', $data, array('pa_meta', 'register_meta_boxes_callback'));
  }
  ...
  public static function clear_screen()
  {
    ...
    $workflow = pa_workflow::get('workflow');

    if ($workflow && !empty($workflow['data']['clear']) && $workflow['data']['clear'] == true && pa_admin::is_post())
    {
  ...
  public static function register_meta_boxes_callback($arguments)
  {
    ...
    $textdomain = isset(pa_add_on::$available_add_ons[$add_on]['text_domain']) ? pa_add_on::$available_add_ons[$add_on]['text_domain'] : null;
    ...
    foreach ($types as $type)
    {
      ...
      elseif (!$data['extend'] || ($data['extend'] && self::update_meta_box($type, $id)) || ($data['extend'] && in_array($data['extend'], self::$meta_boxes_removed)))
      {
        ...
        {
          add_filter("postbox_classes_{$type}_{$id}", array('pa_meta', 'lock_meta_boxes'));
          add_filter("postbox_classes_{$type}_{$id}", array('pa_meta', 'no_meta_boxes'));
        }
        else
        {
          if ($data['lock'] === true)
          ...
          {
            add_filter("postbox_classes_{$type}_{$id}", array('pa_meta', 'collapse_meta_boxes'));
          }
        }

        if ($title == $id)
        {
          add_filter("postbox_classes_{$type}_{$id}", array('pa_meta', 'no_title_meta_boxes'));
        }

        add_filter("postbox_classes_{$type}_{$id}", array('pa_meta', 'default_classes'));
      }
    }
  }
  ...
  {
    do_action('pa_pre_render_meta_box', $post, $meta_box);
    ...
    do_action('pa_post_render_meta_box', $post, $meta_box);
  }
    ...
    if (in_array($pagenow, array('edit.php', 'post.php', 'post-new.php')) && post_type_exists(get_post_type()))
    {
      ...
      foreach (array('side', 'normal', 'advanced') as $context)
      {
        foreach (array('high', 'sorted', 'core', 'default', 'low') as $priority)
        {
          ...
          {
            uasort($wp_meta_boxes[$typenow][$context][$priority], array('pa', 'sort_by_args_order'));
          }
        }
      }
    }
  ...
  {
    array_push($classes, 'pa-meta-box-lock');
  ...
  {
    array_push($classes, 'pa-meta-box-no-title');
  ...
  {
    array_push($classes, 'pa-meta-box-none');
  ...
  /**
   * default_classes
   * Returns classes to be used by a metabox, to identify it as a meta-box created by Pa.
   *
   * @param string $classes The classes for the meta box.
   *
   * @return string The classes for the meta box.
  ...
  {
    array_push($classes, 'pa-meta-box');
  ...
  {
    array_push($classes, 'pa-meta-box-collapse');
  ...
  public static function meta_grouped()
  {
    ...
    foreach (self::$grouped_meta_keys as $meta_type => $meta_keys)
    {
      ...
      {
        $prefix = trim($wpdb->prepare('%s', pa::$prefix), "'");
        $group_keys = $wpdb->get_col("SELECT DISTINCT meta_key FROM $meta->table WHERE meta_key LIKE '\\_\\{$prefix}%'");

        foreach ($group_keys as $group_key)
        {
          $key = $wpdb->get_var($wpdb->prepare("SELECT DISTINCT meta_key FROM $meta->table WHERE meta_key = %s", str_replace('_' . pa::$prefix, '', $group_key)));
  ...
  public static function meta_reset()
  {
    ...
    /**
     * pa_reset_meta_admin_pages
     *
     * @since 1.0
     */
    self::$reset_meta = apply_filters('pa_reset_meta_admin_pages', self::$reset_meta);
  ...
  public static function meta_sort($query)
  {
    ...
    if (stristr($query, ', meta_key, meta_value FROM'))
    {
       ...
       * @since 1.0
       */
      $pa_meta_tables_sort = apply_filters('pa_meta_tables_sort', array());

      // Do not allow filter to overwrite $default_meta_tables_sort
      $meta_tables = array_merge($pa_meta_tables_sort, $default_meta_tables_sort);
  ...
  public static function get_metadata($value, $meta_type, $object_id, $meta_key, $single)
  {
    ...
    $meta_key = '_' . pa::$prefix . $meta_key;

    if (is_array(self::$grouped_meta_keys[$meta_type]) && in_array($meta_key, self::$grouped_meta_keys[$meta_type]))
    {
      remove_filter('get_post_metadata', array('pa_meta', 'get_post_meta'), 100);
      remove_filter('get_user_metadata', array('pa_meta', 'get_user_meta'), 100);
      remove_filter('get_term_metadata', array('pa_meta', 'get_term_meta'), 100);
