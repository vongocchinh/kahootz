...
class Pa_Taxonomy
{
  ...
  public static function _construct()
  {
    ...
    add_action('pa_activate', array('pa_taxonomy', 'activate'));

    add_action('admin_init', array('pa_taxonomy', 'register_meta_boxes'), 50);
    add_action('registered_taxonomy',  array('pa_taxonomy', 'registered_taxonomy'), 10, 3);
    add_action('admin_menu', array('pa_taxonomy', 'admin_menu'));

    add_filter('wp_redirect', array('pa_taxonomy', 'wp_redirect'), 10, 2);
    add_filter('parent_file', array('pa_taxonomy', 'parent_file'));
    add_filter('sanitize_user', array('pa_taxonomy', 'restrict_username'));
    add_filter('pa_meta_tables_sort', array('pa_taxonomy', 'pa_meta_tables_sort'));

    // Load before termmeta was native to WordPress
    if ( version_compare($wp_version, '4.4.0', '<') )
    {
      add_filter('init', array('pa_taxonomy', 'register_tables'));
      add_filter('terms_clauses', array('pa_taxonomy', 'terms_clauses'), 10, 3);
      add_filter('get_terms_args', array('pa_taxonomy', 'get_terms_args'), 0);
    }
  }
   ...
   * @since 1.0
   */
  public static function pa_meta_tables_sort($meta_tables)
  {
    global $wpdb;
  ...
  public static function register_meta_boxes()
  {
    ...
    pa::process_parts('terms', $data, array('pa_taxonomy', 'register_meta_boxes_callback'));
  }
  ...
  public static function register_meta_boxes_callback($arguments)
  {
    ...
    foreach ($taxonomies as $taxonomy)
    {
      ...
      if (!isset(self::$meta_boxes[$data['taxonomy']]))
      {
        ...
        add_action($data['taxonomy'] . '_edit_form_fields', array('pa_taxonomy', 'meta_box_edit'), 10, 2);
        add_action($data['taxonomy'] . '_add_form_fields', array('pa_taxonomy', 'meta_box_new'), 10, 1);
      }
  ...
  public static function meta_box($taxonomy, $new = false)
  {
    if ($taxonomy)
    {
      ...
      uasort(self::$meta_boxes[$taxonomy], array('pa', 'sort_by_data_order'));

      foreach (self::$meta_boxes[$taxonomy] as $_taxonomy => $meta_box)
      {
        if ($meta_box['data']['new'] == $new)
        {
          pa::render('shared/meta-box-seperator', array(
            'meta_box' => $meta_box
  ...
  public static function registered_taxonomy($taxonomy, $object_type, $arguments)
  {
    ...
    if ($object_type == 'user')
    {
      ...
      add_filter("manage_edit-{$taxonomy}_columns",  array('pa_taxonomy', 'user_taxonomy_column'));

      add_action("manage_{$taxonomy}_custom_column",  array('pa_taxonomy', 'user_taxonomy_column_value'), 10, 3);

      if (empty($arguments->update_count_callback))
      {
        $arguments->update_count_callback  = array('pa_taxonomy', 'user_update_count');
      }
  ...
  /**
   * wp_redirect
   * Handle redirects of the edit tags form with pa fields.
   *
