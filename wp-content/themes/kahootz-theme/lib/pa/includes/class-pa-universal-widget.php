 ...
 * @since       1.0
 */
class Pa_Universal_Widget extends WP_Widget
{
  /**
   ...
   * @access private
   */
  public $widget_core_name = 'pa_universal_widget';
  ...
  public function __construct($name, $title = '', $description = '', $customize_selective_refresh = false, $widgets = array(), $control_options = array())
  {
    ...
    $class = parent::__construct(
      pa::dashes($this->widget_name)
      ,__($title)
      ,array(
        'classname' => pa::dashes($this->widget_core_name)
    ...
    {
      do_action('pa_widget_active', $this->widget_name, $widgets);
    }
     
    add_filter('pa_part_id-widgets', array(&$this, 'part_id'), 10, 4);
  ...
  {
    pa_widget::$current_widget = $widget;
  }
  ...
  public function form($instance)
  {
    ...
    $forms = pa_widget::get('widget_forms');
    ...
    {
      pa::render('shared/widget-select', array(
        'instance' => $instance
        ,'widgets' => $this->widgets
        ,'forms' => $forms
        ,'name' => $this->widget_core_name
        ,'widget_name' => $this->widget_name
        ,'class_name' => pa::dashes($this->widget_core_name)
  ...
  public function ajax()
  {
    ...
    if ($widget && current_user_can('edit_theme_options'))
    {
      ...
      {
        $instances = get_option('widget_' . pa::dashes($this->widget_name));

        pa_widget::widget()->_set($_REQUEST['number']);

        if (isset($instances[$_REQUEST['number']]))
        {
          pa_widget::widget()->instance = $instances[$_REQUEST['number']];
        }
      }
  ...
  public function update($new_instance, $old_instance)
  {
    ...
    $this->setup(pa::slug($_REQUEST['id_base']));

    $check = pa_validate::check($request_data);
  ...
  public function widget($arguments, $instance)
  {
    ...
    do_action('pa_pre_render_widget', $this->widgets[$widget]);
    ...
    foreach ($this->widgets[$widget]['render'] as $render)
    {
      if (!strstr(strtolower($render), '-form.php'))
      {
         pa::render($render, array(
          'instance' => $instance
          ,'settings' => $instance
          ,'before_widget' => str_replace('class="', 'class="' . pa::dashes($this->widgets[$widget]['id']) . ' ' . $this->widgets[$widget]['data']['class'] . ' ', $before_widget)
    ...
    do_action('pa_post_render_widget', $this->widgets[$widget]);
  }
  ...
  {
    return pa::slug($add_on . ' ' . str_replace('-form.php', '.php', strtolower($part)));
  }
}