...
{
  /**
   * @var string The current version of Pa.
   * @access public
   */
  public static $version;

  /**
   * @var array Data for all the areas Pa can be found.
   * @access public
   */
  public static $add_ons = array();

  /**
   * @var array The urls to all areas Pa can be found.
   * @access public
   */
  public static $urls = array();

  /**
   * @var array The paths to all areas Pa can be found.
   * @access public
   */
  ...
  /**
   * @var string The prefix used for all pa field names and ids.
   * @access public
   */
  ...
  {
    self::add_plugin('pa', dirname(dirname(__FILE__)));

    self::$version = current(self::get_file_data(self::$add_ons['pa']['path'] . '/pa.php', array('version' => 'Version')));

    load_plugin_textdomain('pa', false, 'pa/languages/');

    register_activation_hook('pa/pa.php', array('pa', 'activate'));

    self::auto_load();

    add_filter('pa_part_data', array('pa', 'part_data'), 10, 2);

    add_action('init', array('pa', 'process_parts_callback'), 1000);
    add_action('admin_init', array('pa', 'process_parts_callback'), 1000);
    add_action('admin_head', array('pa', 'process_parts_callback'), 1000);
    add_action('template_redirect', array('pa', 'process_parts_callback'), 0);
    add_action('widgets_init', array('pa', 'process_parts_callback'), 50);

    add_filter('pa_workflow_part_exclude_folders', array('pa', 'part_exclude_folders'), 10, 3);
  }
   ...
   * @since 1.0
   */
  public static function auto_load($addon = 'pa')
  {
    if (isset(self::$add_ons[$addon]['path']))
    {
      ...
      foreach ($includes as $include)
      {
        ...
        if ($include != __FILE__ && !in_array($class_name, array('pa_universal_control')))
        {
  ...
  {
    pa::check_network_propagate('do_action', 'pa_activate');
  }
  ...
  public static function add_plugin($type, $path)
  {
     ...
     * @since 1.0
     */
    do_action('pa_plugin_loaded-' . $type);
  }
  ...
  public static function render($view, $arguments = array(), $return = false, $loop = null)
  {
    ...
    else
    {
      ...
      $_origin = apply_filters('pa_render_origin', $_origin, $view, $arguments, $return, $loop);
    ...
    if (!isset($_file))
    {
      ...
      if ($view_is_absolute && (false !== strpos($view, '/parts/')) && preg_match('~(?<base>.+(?:pa)?)/parts/(?<view>.+)$~i', $view, $_matches))
      {
    ...
    foreach ($_arguments as $_object)
    {
      foreach ($_object as $_key => $_value)
      {
        ...
        {
          $trigger_error_message = sprintf(__('is a reserved WordPress global variable and cannot be passed as an argument to %s', 'pa'), 'pa::render()');
     ...
     * @since 1.0
     */
    $view = apply_filters('pa_render', $view, $arguments);
    ...
    else
    {
      ...
      {
        trigger_error(sprintf(__('File does not exist%s', 'pa'), ': ' . $view), E_USER_WARNING);
      }
    }
  ...
  public static function process_parts($folder, $data = array(), $callback = null, $path = null, $prefix = '', $suffix = '.php')
  {
     ...
     * @since 1.0
     */
    $data = array_merge(apply_filters('pa_part_data', $data, $folder), $data);
    ...
    foreach ($paths as $add_on => $path)
    {
      ...
      foreach ($files as $part)
      {
        if (self::strtolower($part) != 'index.php' && substr($part, 0, strlen($prefix)) == $prefix && substr($part, strlen($part) - strlen($suffix)) == $suffix)
        {
          ...
          $_part = array(
            'id' => apply_filters("pa_part_id-{$folder}", pa::slug($add_on . ' ' . $part), $add_on, $part, $part_data)
            ,'folder' => $folder
            ,'part' => $part
            ,'data' => $part_data
            ,'prefix' => $prefix
          ...
          if (!empty($part_data['extend']))
          {
            ...
            $_part_extend['data'] = pa::cast_file_data($_part_extend['data']);
          ...
          if ($_part)
          {
             ...
             * @since 1.0
             */
            $_part = apply_filters('pa_part_process', $_part, $folder);

            /**
             * pa_part_process-FOLDER
             ...
             * @since 1.0
             */
            $_part = apply_filters("pa_part_process-{$folder}", $_part);
    ...
    uasort(self::$processed_parts[$folder]['parts'], array('pa', 'sort_by_data_extend'));
     ...
     * @since 1.0
     */
    $parts_added = apply_filters('pa_part_add', array(), $folder);
     ...
     * @since 1.0
     */
    $parts_added = apply_filters("pa_part_add-{$folder}", array());
     ...
     * @since 1.0
     */
    do_action('pa_parts_process', $folder);

    /**
     * pa_parts_process-FOLDER
     * Signals that parts are in process.
     *
     * @param  array $processed parts so far.
     * @param  var $folder the parts folder where the file is located.
  ...
  public static function process_parts_callback()
  {
    ...
    foreach ($processed_parts as $folder => $processed)
    {
      foreach ($processed['parts'] as $part)
      {
         ...
         * @since 1.0
         */
        $part = apply_filters('pa_part_process_callback', $part, $folder);

        /**
         * pa_part_process_callback-FOLDER
         * Post-process for a part by folder.
         *
         * @param  array $part being validated
         *
         * @since 1.0
         */
        $part = apply_filters("pa_part_process_callback-{$folder}", $part);
      ...
      do_action('pa_parts_processed', $folder);
  ...
  public static function cast_file_data($data) {
    array_walk_recursive($data, array('pa', 'array_values_cast'));
    ...
    foreach ($data as $parameter => &$value)
    {
      switch ($parameter)
      {
        ...
        case 'post_format':
          ...
          $value = pa::explode(',', $value, array('pa', 'strtolower'));
        ...
        case 'sub_tab':
          ...
          $value = pa::explode(',', $value, array('pa', 'slug'));
          $value = array_filter($value);
          $value = empty($value) ? null : $value;
        ...
        default:
           ...
           * @since 1.0
           */
          $value = apply_filters('pa_part_data_parameter', $value, $parameter);
  ...
  public static function validate_part($part)
  {
    if (isset($part['data']))
    {
      foreach ($part['data'] as $parameter => $value)
      {
        if (!empty($value))
        {
          if (!self::validate_part_parameter($parameter, $value) && !apply_filters('pa_validate_part_parameter_skip', false, $parameter, $part))
          {
            return false;
          }
        }
  ...
  public static function validate_part_parameter($parameter, $value)
  {
    ...
    switch ($parameter)
    {
      case 'capability':
        ...
        return pa_user::current_user_can($value);

      break;

      case 'role':

        return pa_user::current_user_role($value);
      ...
      default:
         ...
         * @since 1.0
         */
        return apply_filters('pa_validate_part_parameter', true, $parameter, $value);
  ...
  public static function post_type_labels($label)
  {
    ...
    $labels = array(
      ...
      ,'all_items' => sprintf(__('All %s', 'pa'), $plural)
      ,'add_new' => __('Add New', 'pa')
      ,'add_new_item' => sprintf(__('Add New %s', 'pa'), $singular)
      ,'edit_item' => sprintf(__('Edit %s', 'pa'), $singular)
      ,'new_item' => sprintf(__('Add New %s', 'pa'), $singular)
      ,'view_item' => sprintf(__('View %s', 'pa'), $singular)
      ,'search_items' => sprintf(__('Search %s', 'pa'), $plural)
      ,'not_found' => sprintf(__('No %s found', 'pa'), $plural)
      ,'not_found_in_trash' => sprintf(__('No %s found in trash', 'pa'), $plural)
      ,'parent_item_colon' => sprintf(__('Parent %s:', 'pa'), $plural)
      ,'menu_name' => $plural
    );

    /**
     * pa_post_type_labels_locale
     * Pass an array of labels for a Post Type
     *
     * Allows you to create new labels for an already registered Post Type that uses pa('post_type_labels')
     ...
     * @since 1.0
     */
    return apply_filters('pa_post_type_labels_locale', $labels, $locale);
  }
  ...
  public static function taxonomy_labels($label)
  {
    ...
    $labels = array(
      ...
      ,'search_items' => sprintf(__('Search %s', 'pa'), $plural)
      ,'all_items' => sprintf(__('All %s', 'pa'), $plural)
      ,'parent_item' => sprintf(__('Parent %s', 'pa'), $plural)
      ,'parent_item_colon' => sprintf(__('Parent %s:', 'pa'), $plural)
      ,'edit_item' => sprintf(__('Edit %s', 'pa'), $singular)
      ,'update_item' => sprintf(__('Update %s', 'pa'), $singular)
      ,'add_new_item' => sprintf(__('Add New %s', 'pa'), $singular)
      ,'view_item' => sprintf(__('View %s', 'pa'), $singular)
      ,'popular_items' => sprintf(__('Popular %s', 'pa'), $plural)
      ,'new_item_name' => sprintf(__('New %s Name', 'pa'), $singular)
      ,'separate_items_with_commas' => sprintf(__('Separate %s with commas', 'pa'), $plural)
      ,'add_or_remove_items' => sprintf(__('Add or remove %s', 'pa'), $plural)
      ,'choose_from_most_used' => sprintf(__('Choose from the most used %s', 'pa'), $plural)
      ,'not_found' => sprintf(__('No %s found', 'pa'), $plural)
      ,'menu_name' => $plural
    ...
    /**
     * pa_taxonomy_labels_locale
     * Pass an array of labels for a Taxonomy
     *
     * Allows you to create new labels for an already registered Taxonomy that uses pa('taxoomy_labels')
     * Primarily used for non-Engish locales
     *
     * @param array $labels
     * @param array $locale
     ...
     * @since 1.0
     */
    return apply_filters('pa_taxonomy_labels_locale', $labels, $locale);
  }
  ...
  */
  ...
  public static function object_to_array($object)
  {
    ...
    return array_map(array('pa', 'object_to_array'), $object);
  }
  ...
  public static function sort_by_data_extend($a, $b)
  {
    ...
    $a_extend = isset($a['data']['extend']) && !empty($a['data']['extend']);
    $b_extend = isset($b['data']['extend']) && !empty($b['data']['extend']);
    
    if ($a_extend === $b_extend) {
      return 0;
    }
    return $a_extend ? 1 : -1;
  }
...
/**
 * pa
 * The core helper function for the Pa framework.
 *
 * @since 1.0
 */
function pa($option, $arguments = array())
{
  ...
  {
    return empty($arguments) ? pa::object_format($option) : pa::pluck($option, $arguments);
  }
  else
  {
    ...
    switch ($option)
    {
      case 'field':
        ...
        if (empty($arguments))
        {
          return pa_form::get('field_rendered');
        }
        else
        {
          if (pa_setting::get('active_section'))
          {
            pa_setting::register_setting($arguments);
          }
          else
          {
            return pa_form::render_field($arguments, isset($arguments['return']) ? $arguments['return'] : false);
          }
        }
      ...
      case 'form':
        ...
        return pa_form::render_form($arguments['form'], isset($arguments['add_on']) ? $arguments['add_on'] : null);

      break;

      case 'list_table':

        pa_list_table::render($arguments);

      break;

      case 'post_type_labels':

        return pa::post_type_labels($arguments);

      break;

      case 'taxonomy_labels':

        return pa::taxonomy_labels($arguments);
      ...
      case 'get_term_custom':
        ...
        return pa::object($type, $arguments);
      ...
      case 'performance':
        ...
        return call_user_func_array("pa::{$option}", $function_arguments);

      break;

      case 'relate':

        call_user_func_array('pa_relate::relate_objects', $function_arguments);

      break;

      case 'unrelate':

        call_user_func_array('pa_relate::unrelate_objects', $function_arguments);
      ...
      case 'comments_template':
        ...
        pa_comments::comments_template($file, $seperate_comments);
      ...
      case 'include_meta_boxes':
        ...
        pa::render('shared/notice', array(
          'content' => sprintf(__('This page is using the old Pa WorkFlow system. Please update your code to the %snew WorkFlow system%s.', 'pa'), '<a href="https://pa.com/user-guide/docs/building-workflows-pa-v0-9-9/" target="_blank">', '</a>')
          ,'notice_type' => 'error'
        ));
      ...
      case 'url':
        ...
        return isset(pa::$add_ons[$arguments]) ? pa::$add_ons[$arguments]['url'] : null;

      break;

      case 'path':

        return isset(pa::$add_ons[$arguments]) ? pa::$add_ons[$arguments]['path'] : null;

      break;

      case 'capture':

        pa::render_capture($arguments);

      break;

      case 'calendar':

        return call_user_func_array('pa::calendar', $function_arguments);

      break;

      case 'choice_tree':
        return call_user_func_array('pa::build_choices_tree', $function_arguments);

      default:

        if ('no-custom-pa-function' !== ($custom_value = apply_filters("pa_function-{$option}", 'no-custom-pa-function', $arguments)))
        {
          return $custom_value;
        }
        ...
        return pa::render($option, $arguments, $return, $loop);
