...
class Pa_Setting
{
  ...
  public static function _construct()
  {
    if (is_admin())
    {
      add_action('admin_init', array('pa_setting', 'register_settings'));
      add_action('current_screen', array('pa_setting', 'add_meta_boxes'));
      add_action('admin_enqueue_scripts', array('pa_setting', 'admin_enqueue_scripts'));
      add_action('pa_parts_processed-settings', array('pa_setting', 'parts_processed'));

      add_filter('pa_admin_pages', array('pa_setting', 'admin_pages'));
      add_filter('pa_part_add-workflows', array('pa_setting', 'part_add'), 10, 2);
      add_filter('pa_part_process-settings', array('pa_setting', 'part_process'), 10, 2);
    }
  }

  /**
   * admin_pages
   * Create default settings pages for Pa.
   *
  ...
  {
    $pages[] = array(
      'page_title' => __('About', 'pa')
      ,'menu_title' => __('Pa', 'pa')
      ,'capability' => defined('PA_SETTINGS_CAP') ? PA_SETTINGS_CAP : 'manage_options'
      ,'menu_slug' => 'pa'
      ,'single_line' => false
      ,'menu_icon' => pa::$add_ons['pa']['url'] . '/parts/img/pa-menu-icon.svg'
      ,'page_icon' => pa::$add_ons['pa']['url'] . '/parts/img/pa-page-icon-32.png'
    );

    $pages[] = array(
      'page_title' => __('Pa Settings', 'pa')
      ,'menu_title' => __('Settings', 'pa')
      ,'capability' => defined('PA_SETTINGS_CAP') ? PA_SETTINGS_CAP : 'manage_options'
      ,'sub_menu' => 'pa'
      ,'menu_slug' => 'pa-core-settings'
      ,'setting' => 'pa_core'
      ,'menu_icon' => pa::$add_ons['pa']['url'] . '/parts/img/pa-menu-icon.svg'
      ,'page_icon' => pa::$add_ons['pa']['url'] . '/parts/img/pa-page-icon-32.png'
      ,'single_line' => true
    ...
    $pages[] = array(
      'page_title' => __('Pa Add-ons', 'pa')
      ,'menu_title' => __('Add-ons', 'pa')
      ,'capability' => defined('PA_SETTINGS_CAP') ? PA_SETTINGS_CAP : 'manage_options'
      ,'sub_menu' => 'pa'
      ,'menu_slug' => 'pa-core-addons'
  ...
  public static function register_settings()
  {
    ...
    pa::process_parts('settings', $data, array('pa_setting', 'register_settings_callback'));
  }
  ...
  {
    $field = pa_form::setup_field($field);

    add_settings_field(
      isset($field['field']) ? $field['field'] : pa::unique_id()
      ,isset($field['label']) ? pa_form::field_label($field) : null
      ,array('pa_setting', 'render_setting')
  ...
  public static function register_settings_section_callback($arguments)
  {
    ...
    do_action('pa_pre_render_setting_section', $section, $options);
    ...
    do_action('pa_post_render_setting_section', $section, $options);
    ...
    if (true === $check['valid'] && $check['type'] == 'POST')
    {
      ...
      foreach ($fields_data[$setting] as &$data)
      {
        if (empty($data['display']))
        {
          ...
          {
            $paths = pa::array_paths($_FILES[$setting]['name'][$data['field']]);
            ...
            if (!empty($paths))
            {
              if (strstr($paths[0], ':'))
              {
                foreach ($paths as $path)
                {
                  ...
                  $upload = pa_form::save_upload($files_path, $storage, $storage_type);

                  if ($upload)
                  {
                    pa::array_path_set($new[$data['field']], explode(':', $path), current($upload));
                  }
                }
              }
              else
              {
                ...
                $upload = pa_form::save_upload($path, $storage, $storage_type);
       ...
       * @since 1.0
       */
      $settings = apply_filters('pa_pre_update_option', $settings, $setting, $new, $old);

      /**
       * pa_pre_update_option_$setting
       ...
       * @since 1.0
       */
      $settings = apply_filters('pa_pre_update_option_' . $setting, $settings, $new, $old);
    }
    else
    {
      $settings = $old;
  ...
  public static function render_setting($setting)
  {
    $field = wp_parse_args(array(
        ...
        ,'disable_label' => true
        ,'position' => false
        ,'value' => pa_form::get_field_value($setting['section']['data']['setting'], $setting['field'], 'option')
      )
      ,$setting['field']);
    ...
    if ($field['type'] == 'group' && !$field['field'])
    {
      foreach ($field['fields'] as &$column)
      {
        ...
        $column['value'] = pa_form::get_field_value($setting['section']['data']['setting'], $column, 'option');
      }
      unset($column);
    }

    pa_form::render_field($field);
  }
  ...
  public static function part_add($parts)
  {
    ...
    if ($page)
    {
      $workflow = array();
      $default_tab = array();
      $process_parts = pa::get_processed_parts('settings');
      ...
      if ($process_parts)
      {
        foreach ($process_parts['parts'] as $part)
        {
          if ($part['data']['setting'] == $page && is_array($part['data']['flow']) && in_array($part['data']['setting'], $part['data']['flow']))
          {
            ...
            {
              $admin_pages = pa_admin::get('admin_pages');
              ...
              $default_tab = array(
                ...
                ,'data' => array(
                  ...
                  ,'page' => array($part['data']['setting'])
                  ,'order' => $part['data']['order']
                  ,'title' => __(isset($admin_page['default_tab']) && $admin_page['default_tab'] ? pa::slug($admin_page['default_tab']) : 'General')
                  ,'position' => 'title'
                  ,'tab' => null
                  ,'post_type' => null
                  ,'header' => false
  ...
  public static function parts_processed()
  {
    ...
    foreach (self::$settings as $setting => $sections)
    {
      ...
      {
        add_filter('pre_update_option_' . $setting, array('pa_setting', 'pre_update_option'), 10, 2);
      }

      register_setting($setting, $setting);

      uasort($sections, array('pa', 'sort_by_data_order'));
      ...
      foreach ($sections as $section)
      {
        ...
        $textdomain = isset(pa_add_on::$available_add_ons[$section['add_on']]['TextDomain']) ? pa_add_on::$available_add_ons[$section['add_on']]['TextDomain'] : null;
        $title = !empty($section['data']['title']) ? $section['data']['title'] : (!empty($id) ? $id : __('Settings', 'pa'));
        $title = !empty($textdomain) ? __($title, $textdomain) : $title;

        if ($active && pa_admin::$admin_page_layout == 'meta-boxes')
        {
          if (!$submitdiv)
          {
            $admin_pages = pa_admin::get('admin_pages');
            ...
            array_push(self::$meta_boxes, array(
              ...
              ,__('Actions')
              ,array('pa_admin', 'add_meta_box_submitdiv_callback')
              ,null
              ,'side'
              ,'high'
              ,$page
          ...
          array_push(self::$meta_boxes, array(
            $section['id']
            ,$title
            ,array('pa_setting', 'add_meta_box_callback')
            ,null
            ,$context
            ,$priority
            ,array(
        ...
        {
          add_settings_section($section['id'], $title, array('pa_setting', 'register_settings_section_callback'), $setting);
        }
      }
    }
  }
  ...
  public static function part_process($part)
  {
    ...
    if ($page && empty($part['data']['flow']))
    {
      ...
      if (!$part['data']['tab'])
      {
        ...
        $part['data']['tab'] = array(isset($admin_page['default_tab']) ? pa::slug($admin_page['default_tab']) : 'general');
      }
    }
