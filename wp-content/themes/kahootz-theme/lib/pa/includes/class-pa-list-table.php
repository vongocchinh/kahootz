...
class Pa_List_Table 
{
  ...
  /**
   * render
   * This method will render a list table using the universal pa list table.
   *
  ...
  public static function render($arguments)
  {
    ...
    $list_table = new Pa_List_Table_Template($arguments);
    $list_table->arguments = $arguments;
    $list_table->prepare_items();
    
    pa::render('shared/list-table', array(
      'list_table' => $list_table
      ,'name' => $name
      ,'export' => isset($export) ? $export : false
      ,'form_id' => 'pa_list_table_form_' . pa::slug($name)
    ));
  }
}
...
/**
 * Pa_List_Table_Template
 * See WP_List_Table for documentation on these methods.
 *
 * @package pa
 */
class Pa_List_Table_Template extends WP_List_Table 
{
  var $data;
  ...
  function __construct($arguments)
  {
    ...
    $this->key = $key;
    $this->name = $name;
    $this->table_id = pa::slug($this->name);
    $this->column = $column;
    ...
    if (isset($data))
    {
      $this->data = is_object($data) || is_array($data) ? pa::object_to_array($data) : $wpdb->get_results($data, ARRAY_A);
    ...
    parent::__construct(array(
      'singular' => pa::singularize($this->name)
      ,'plural' => pa::pluralize($this->name)
      ,'ajax' => $this->ajax
   ));
  }
  ...
  function pagination($which) 
  {
    ...
    $output = '<span class="displaying-num">' . sprintf(_n('1 item', '%s items', $total_items, 'pa'), number_format_i18n($total_items)) . '</span>';
    ...
    $page_links[] = sprintf(
      ...
      ,esc_attr__('Go to the first page', 'pa')
    ...
    $page_links[] = sprintf(
      ...
      ,esc_attr__('Go to the previous page', 'pa')
    ...
    $html_current_page = sprintf(
      ...
      ,esc_attr__('Current page', 'pa')
      ,$current
      ,strlen($total_pages)
    );
    
    $html_total_pages = sprintf("<span class='total-pages'>%s</span>", number_format_i18n($total_pages));
    $page_links[] = '<span class="paging-input">' . sprintf(_x('%1$s of %2$s', 'paging', 'pa'), $html_current_page, $html_total_pages) . '</span>';

    $page_links[] = sprintf(
      "<a class='%s' title='%s' href='%s'>%s</a>"
      ,'next-page' . $disable_last
      ,esc_attr__('Go to the next page', 'pa')
    ...
    $page_links[] = sprintf(
      ...
      ,esc_attr__('Go to the last page', 'pa')
