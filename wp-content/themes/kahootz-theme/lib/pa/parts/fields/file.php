...
<div class="pa-field-part">
  ...
  <?php else: ?>
    ...
    <a
      href="#"
      class="button pa-upload-file-button pa-field-part <?php echo $errors ? 'pa-error' : null; ?>"
  ...
  <div class="pa-upload-file-preview pa-field-preview">
    ...
    foreach($value as $attachment)
    {
      if ($attachment)
      {
        if (is_numeric($attachment))
        {
          ...
          if (false !== $type)
          {
            $check = pa_media::image_has_size($attachment_id, $options['preview_size']);
            array_push($attachments, array(
              'id' => $attachment_id
              ,'type' => $type
              ,'data' => pa_media::image_has_size($attachment_id, $options['preview_size'])
        ...
        else
        {
          ...
          {
            $image_sizes = pa_media::image_has_size($attachment_id, $options['preview_size'])
              ? pa_media::get_image_sizes($options['preview_size'])
              : pa_media::get_image_sizes('thumbnail');
                ...
                <li class="attachment selected" <?php echo $image[1] ? 'style="width: ' . $image[1] . 'px;"' : null; ?>>
                   <div class="attachment-preview <?php echo (int) $image[1] > (int) $image[2] ? 'landscape' : 'portrait'; ?>">
                     ...
                     <?php if (version_compare($wp_version, '4.3', '<')): ?>
                       <a class="check" href="#" title="Deselect" tabindex="0" data-attachment-save="<?php echo $options['save']; ?>" data-attachment-id="<?php echo $attachment['id']; ?>" data-attachment-url="<?php echo esc_attr($image[0]); ?>" data-attachments="<?php echo pa_form::get_field_name($arguments); ?>"><div class="media-modal-icon"></div></a>
                     <?php else: ?>
                       <button type="button" class="button-link check" data-attachment-save="<?php echo $options['save']; ?>" data-attachment-id="<?php echo $attachment['id']; ?>" data-attachment-url="<?php echo esc_attr($image[0]); ?>" data-attachments="<?php echo pa_form::get_field_name($arguments); ?>"><span class="media-modal-icon"></span><span class="screen-reader-text"><?php _e('Deselect'); ?></span></button>
                     <?php endif; ?>
                   </div>
                 </li>
      ...
      ?>
                ...
                <li class="attachment selected">
                   <div class="attachment-preview attachment-preview-document landscape type-<?php echo substr($file_type['type'], 0, strpos($file_type['type'], '/')); ?> subtype-<?php echo substr($file_type['type'], strpos($file_type['type'], '/') + 1); ?>">
                     ...
                     <?php if (version_compare($wp_version, '4.3', '<')): ?>
                       <a class="check" href="#" title="Deselect" tabindex="0" data-attachment-save="<?php echo $options['save']; ?>" data-attachment-id="<?php echo $attachment['id']; ?>" data-attachments="<?php echo pa_form::get_field_name($arguments); ?>"><div class="media-modal-icon"></div></a>
                     <?php else: ?>
                       <button type="button" class="button-link check" data-attachment-save="<?php echo $options['save']; ?>" data-attachment-id="<?php echo $attachment['id']; ?>" data-attachments="<?php echo pa_form::get_field_name($arguments); ?>"><span class="media-modal-icon"></span><span class="screen-reader-text"><?php _e('Deselect'); ?></span></button>
                     <?php endif; ?>
                   </div>
                 </li>
