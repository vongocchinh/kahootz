(function($) 
{
  tinymce.create('tinymce.plugins.PaShortcodePlugin', {
      ...
      $.each(pa.shortcodes, function(index, shortcode) 
      {
        if (shortcode.editor)
        {
          editor.addButton('shortcode-' + shortcode.shortcode, {
            ...
            icon: 'dashicon dashicons ' + shortcode.icon + ' pa-shortcode-button',
            role: shortcode.shortcode,
            onclick: function(event) 
            {
              var button = $(event.target).parents('.mce-btn:eq(0)');

              if (!button.hasClass('pa-shortcode-button'))
              {
                button
                  .addClass('pa-shortcode-button')
                  .find('.pa-shortcode-button:eq(0)')
                  .removeClass('pa-shortcode-button');
              }
            }
          });
        }
    ...
    {
      return {
        longname: 'Pa Shortcode Plugin',
        author: 'Pa',
        authorurl: 'https://pa.com',
        infourl: 'https://pa.com',
        version: tinymce.majorVersion + '.' + tinymce.minorVersion
      };
    }
  });
 
  tinymce.PluginManager.add('pa_shortcode', tinymce.plugins.PaShortcodePlugin);
  
  function reset_views_callback(match, view) 
  {
    var shortcode = $(match).data('wpview-type');
  
    return pa.shortcodes[shortcode].inline === true ? window.decodeURIComponent(view) : '<p>' + window.decodeURIComponent(view) + '<\/p>';
  }
