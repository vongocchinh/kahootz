/* --------------------------------------------------------------------------------
  Updates or enhancements to Pa Functionality
--------------------------------------------------------------------------------- */
...
;(function($, window, document, undefined)
{
  ...
  var pa_admin = {
    ...
    init: function()
    {
      ...
      pa_admin.meta_boxes();
      pa_admin.post_name();
      pa_admin.post_submit_meta_box();
      pa_admin.thickbox();
      pa_admin.user_forms();
      pa_admin.tag_forms();
      pa_admin.empty_elements();
      pa_admin.list_tables();
      pa_admin.widgets();
      pa_admin.customizer();
      pa_admin.shortcodes();
      pa_admin.notices();
    ...
    pointers: function()
    {
      $.each(pa.pointers, function(index)
      {
    ...
    add_ons: function()
    {
      if ($('.pa-field-add-on-button').length > 0)
      {
        $('.pa-field-add-on-button').click(function(event)
        {
    ...
    thickbox: function()
    {
      ...
      $(document).on('click', 'body.pa-in-iframe .pa-upload-file-button, body.pa-in-iframe .wp-media-buttons button', function()
      {
      ...
      $(document).on('click', '.pa-list-table-export-button', function()
      {
        setTimeout(function()
        {
          var TB_WIDTH = 870,
    ...
    meta_boxes: function()
    {
      $('.pa-meta-box-collapse:not(.pa-meta-box-lock)').addClass('closed');
      ...
      $('.pa-meta-box-lock').each(function()
      {
        ...
        $(this)
          .find('h2.hndle')
          .removeClass('hndle')
          .addClass('pa-meta-box-hndle');
      });

      $('.pa-meta-box > .inside').each(function()
      {
        if ($(this).find(' > *:first-child').hasClass('pa-field-container'))
        {
          $(this).css({
            'margin-top': '0'
          });
    ...
    {
      if (typeof pa.post != 'undefined' && typeof pa.post_statuses != 'undefined' && $('div#submitdiv.postbox').length > 0)
      {
        ...
        $(':input#original_post_status').val(pa.post.post_status);

        post_status.children().each(function()
        {
          var value = $(this).val();

          if (typeof pa.post_statuses != 'undefined')
          {
            $(this).remove();
          }
        });

        for (var status in pa.post_statuses)
        {
          post_status.append('<option value="' + status + '" ' + (status == pa.post.post_status ? 'selected="selected"' : '') + '>' + pa.post_statuses[status].label + '</option>');
          ...
          if (!default_status)
          {
            default_status = pa.post_statuses[status];
          }
        }

        $('span#post-status-display').text(pa.post.post_status != 'auto-draft' && typeof pa.post_statuses[pa.post.post_status] != 'undefined' ? pa.post_statuses[pa.post.post_status].label : default_status.label)
    ...
    shortcodes: function()
    {
      $.each(pa.shortcodes, function(index, shortcode)
      {
        if (wp.mce.views)
        {
          var tag = typeof shortcode.shortcode != 'undefined' ? shortcode.shortcode : shortcode;

          wp.mce.views.register(tag, pa_admin.shortcode(tag));
        }
      });
      ...
      $(document).on('click', '.pa-shortcode-button', function(event)
      {
        event.preventDefault();
        ...
        if ($(this).hasClass('mce-btn'))
        {
          ...
          attributes[pa.prefix + 'shortcode_data[name]'] = $(this).attr('role');
          attributes[pa.prefix + 'shortcode_data[action]'] = 'insert';
        }
        else if ($(this).hasClass('mce-ico'))
        {
          title = $(this).parents('.mce-btn').attr('aria-label');

          attributes[pa.prefix + 'shortcode_data[name]'] = $(this).parents('.mce-btn').attr('role');
          attributes[pa.prefix + 'shortcode_data[action]'] = 'insert';
        }
        else if ($(this).data('pa-shortcode'))
        {
          attributes[pa.prefix + 'shortcode_data[name]'] = $(this).data('pa-shortcode');
          attributes[pa.prefix + 'shortcode_data[action]'] = 'insert';
        }

        if (typeof pagenow != 'undefined' && $.inArray(pagenow, ['post', 'post-new']) > -1)
        {
          attributes[pa.prefix + 'post[ID]'] = $('#post_ID').val();
        }

        attributes[pa.prefix + '[admin_hide_ui]'] = 'true';

        if (content && !frame_in_frame)
        {
          attributes[pa.prefix + 'shortcode_data[content]'] = content;
        }
      ...
      {
        if (pa.validate_check)
        {
          var data = $(':input[name^="' + pa.prefix + 'shortcode_data["]').serializeArray(),
            shortcode = {
              attrs: {},
              type: 'single'
            },
          ...
          {
            attribute = data[i].name.replace(pa.prefix + 'shortcode_data[', '').replace(/[\[\]']+/g, '');
          ...
          data = $(':input[name^="' + pa.prefix + 'shortcode["]').serializeArray();

          for (var i = 0; i < data.length; i++)
          {
            attribute_length = (pa.prefix + 'shortcode').length + 1;
          ...
          if (typeof pa.shortcodes[shortcode.tag] != 'undefined' && (pa.shortcodes[shortcode.tag].type == 'closed' || pa.shortcodes[shortcode.tag].type == 'single'))
          {
            shortcode.type = pa.shortcodes[shortcode.tag].type;
          }

          output = wp.shortcode.string(shortcode);

          var _output = $(document).triggerHandler('pa:shortcode:insert', [output, shortcode]);
        ...
        {
          $('ul.pa-shortcodes > .attachment').on('click', function(event)
          {
            $('input[name="' + pa.prefix + 'shortcode_data[name]"]').val($(this).data('pa-shortcode'));

            var data = $(':input[name^="' + pa.prefix + 'shortcode_data["]').serializeArray(),
              post_id = typeof pagenow != 'undefined' && $.inArray(pagenow, ['post', 'post-new']) > -1 ? '&' + pa.prefix + 'post[ID]=' + $('#post_ID').val() : null,
              attributes = {};
    ...
    {
      return {
        template: wp.media.template('pa-shortcode'),

        getContent: function()
        {
          if (pa.shortcodes[this.shortcode.tag].preview === true)
          {
            var is_IE = typeof tinymce != 'undefined' ? tinymce.Env.ie : false,
              preview = $('<iframe/>'),
              id = 'pa-shortcode-preview-' + this.shortcode.tag + '-' + Math.random().toString(36).substr(2, 16);
          ...
          else if (pa.shortcodes[this.shortcode.tag].inline === true)
          {
            var preview = [];

            this.template = wp.media.template('pa-shortcode-inline');
            ...
            return this.template({
              ...
              preview: preview,
              options: typeof pa.shortcodes[this.shortcode.tag] != 'undefined' ? pa.shortcodes[this.shortcode.tag] : {
        ...
        replaceMarkers: function()
        {
          this.getMarkers(function(editor, node)
          {
            ...
            if (pa.shortcodes[this.type].inline === true)
            {
        ...
        edit: function(string)
        {
          ...
          if (match)
          {
            ...
            {
              attributes[pa.prefix + 'shortcode[' + key + ']'] = value;
            });

            attributes[pa.prefix + 'shortcode_data[name]'] = shortcode.tag;
            attributes[pa.prefix + 'shortcode_data[action]'] = 'update';
            attributes[pa.prefix + 'shortcode_data[index]'] = index < 0 ? false : index;

            if (typeof shortcode.content != 'undefined')
            {
              attributes[pa.prefix + 'shortcode_data[content]'] = switchEditors._wp_Nop(shortcode.content);
            }
            ...
            tb_show('Edit ' + (pa.shortcodes[this.type].name ? pa.shortcodes[this.type].name : null), editor_url + '?' + $.param(attributes));
    ...
    customizer: function()
    {
      if ($('body').hasClass('wp-customizer'))
      {
        ...
        {
          setTimeout(pa_admin.customizer_widget_setup, 100);
        });
        ...
        {
          if (widget.parent().attr('id').indexOf('customize-control-widget' + pa.prefix + 'pa-universal-widget') > -1)
          {
            ...
            widget_container
              .removeData('wptabs')
              .removeData('pagroups')
              .removeData('pacolumns')
              .removeData('pamediaupload')
              .removeData('paaddmore')
              .removeData('pafields');

            setTimeout(function()
            {
              widget_container
                .find('.pa-universal-widget-form-container')
                .wptabs()
                .pagroups()
                .pacolumns()
                .pamediaupload()
    ...
    {
      $('.pa-universal-widget-select').each(function()
      {
        $(this)
    ...
    widgets: function()
    {
      $(document).on('mousedown', '.widget input[name="savewidget"]', function()
      {
        var button = $(this),
          ...
          title = button.parents('form').find('.pa-universal-widget-form-container').data('widget-title');

        $('.pa-universal-widget-form-container').on('remove', function()
        {
          widget_container
            .css({
      ...
      pa_admin.widget_title();

      $(document).on('change', '.pa-universal-widget-select', function(event)
      {
        var widget = $(this).val(),
          addon = $(this).data('pa-addon'),
          action = ('pa-universal-widget-' + addon).replace(/-/g, '_'),
          widget_container = $(this).parents('.widget-content'),
          widget_classes = $(this).attr('class').split(' '),
          widget_form = widget_container.find('.pa-universal-widget-form-container'),
          widget_number = $(this).attr('name').split('[')[1].replace(/\]/g, ''),
          widget_title = $(this).parents('.widget').find('.widget-title h4'),
          widget_description = widget_container.find('.pa-universal-widget-select-container p'),
          wptab_active = widget_container.data('pa-wptab-active');
        ...
        if (widget)
        {
          ...
          $.ajax({
            ...
            ,success: function(response)
            {
              ...
              pa_admin.widget_dimensions(widget_container, response.widget.data.height, response.widget.data.width);
            }
          });
        }
      });
      ...
      $('.wp-tab-bar li a').on('click', function(event)
      {
        ...
        {
          widget_container.attr('data-pa-wptab-active', $(this).text());
        }
      });
    ...
    widget_title: function()
    {
      setTimeout(function()
      {
        $('.pa-universal-widget-form-container').each(function()
        {
          ...
          pa_admin.widget_dimensions(widget_container, height, width);
        });
      }, 250);
    },
    ...
    {
      $('.pa-list-table-export-columns')
        .sortable()
        .disableSelection();

      $('.pa-list-table-export-submit').on('click', function(event)
      {
  ...
  $(document)
    .on('sortstart', '.ui-sortable', function(event, ui)
    {
      ...
      {
        $(this).find('.wp-editor-area.pa-field-element').each(function()
        {
          var id = $(this).attr('id'),
